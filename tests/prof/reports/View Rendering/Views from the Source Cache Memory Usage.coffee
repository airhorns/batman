...
keys = ["view memory usage: source cache"]
...
points = new Summarizer(query "SELECT Points.x, Points.y, Reports.sha FROM Points LEFT JOIN Reports ON (Points.ReportId = Reports.id) WHERE Reports.key IN (#{qs(keys.length)}) AND Reports.sha IN (?, ?, ?, ?)", keys..., shas...)
