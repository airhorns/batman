...
(function() {
    ...
    Batman.Controller = Batman.Mixin({
        ...
        routePrefix: '#!',
        route: Batman.binding().observe(function(routeString) {
            window.location.hash = this.routePrefix + routeString;
            
            if (!this._routeFunctionCalled) {
                var route = this.matchRoute(routeString);
                if (route)
                    route.route.dispatch(route.params);
                else
                    this.route('/404');
            }
        ...
        addRoute: function(match, route) {
            var regexp = match.replace(escapeRegExp, '\\$&')
                              .replace(namedParam, '([^\/]*)')
                              .replace(splatParam, '(.*?)');
            
            this.routes.push({match: new RegExp('^' + regexp + '$'), route: route});
        },
        ...
        matchRoute: function(match) {
            ...
            for (var i = -1, count = routes.length; ++i < count;) {
                ...
                if (route.match.test(match))
                    return {route: route.route, params: route.match.exec(match).slice(1), match: match};
            }
        }
    });
