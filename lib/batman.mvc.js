...
(function() {
    ...
    Batman.App = Batman.Mixin({
        ...
        route: Batman.event(function() {
            ...
            if (Batman.Controller.routeDescriptors.length)
                hashChanged();
        }),
    });
    ...
    Batman.Controller = Batman.Mixin({
        ...
        route: Batman.binding().observe(function(match) {
            ...
            if (!this._routeFunctionCalled) {
                var descriptor = this.matchRoute(match);
                if (descriptor)
                    descriptor.route.dispatch(this.extractParams(match, descriptor));
                else
                    this.route('/404');
            }
        ...
        addRoute: function(match, route) {
            match = match.replace(escapeRegExp, '\\$&');
            
            var regexp = match.replace(namedParam, '([^\/]*)')
                              .replace(splatParam, '(.*?)');
            
            var array, paramNames = [];
            while ((array = namedOrSplat.exec(match)) !== null) {
                paramNames.push(array[1]);
            }
            
            var descriptor = {match: new RegExp('^' + regexp + '$'), route: route, paramNames: paramNames};
            this.routeDescriptors.push(descriptor);
        },
        
        matchRoute: function(match) {
            var descriptors = this.routeDescriptors, descriptor;
            for (var i = -1, count = descriptors.length; ++i < count;) {
                descriptor = descriptors[i];
                
                if (descriptor.match.test(match))
                    return descriptor;
            }
        },
        
        extractParams: function(match, descriptor) {
            var array = descriptor.match.exec(match).slice(1),
                params = {};
            
            for (var i = -1, count = array.length; ++i < count;)
                params[descriptor.paramNames[i]] = array[i];
    ...
    Batman.Route = Batman.Mixin({
        ...
        dispatch: function(params) {
            ...
            if (typeof this.action === 'function')
                result = this.action.apply(this.context || this, arguments);
