    ...
    Batman.Route = Batman.Mixin({
        ...
        dispatch: function(params) {
            ...
            if (typeof result === 'string') {
                ...
                result = Batman.View({node: node});
            }
            
            if (result && result.isView) {
                if (lastRenderedView.readyCallback)
                    lastRenderedView.view.ready.forget(lastRenderedView.readyCallback);
                
                lastRenderedView.view = result;
                lastRenderedView.readyCallback = function() {
                    var node = result.node();
                    node && Batman.DOM.bindings.contentFor('main', node);
                };
                
                result.ready(lastRenderedView.readyCallback);
            }
