(function() {
  ...
  applyExtra = function(Batman) {
    ...
    return Batman.RailsStorage = (function() {
      ...
      RailsStorage.prototype._addJsonExtension = function(_arg) {
        var err, url;
        err = _arg[0], url = _arg[1];
        if (!err) {
          url += '.json';
        }
        return [err, url];
      ...
      RailsStorage.prototype.after('update', 'create', function(_arg, next) {
        var error, key, record, response, validationError, validationErrors, _i, _len, _ref, _ref2;
        error = _arg.error, record = _arg.record, response = _arg.response;
        if (error) {
          if (((_ref = error.request) != null ? _ref.get('status') : void 0) === 422) {
            _ref2 = JSON.parse(response);
            for (key in _ref2) {
              validationErrors = _ref2[key];
