(function() {
  var $block, $event, $eventOneShot, $findName, $get, $mixin, $redirect, $typeOf, $unmixin, Batman, BatmanObject, Binding, RenderContext, Validators, buildParams, buntUndefined, camelize_rx, capitalize_rx, container, filters, helpers, k, mixins, param, prefixes, r20, rbracket, underscore_rx1, underscore_rx2, _Batman, _class, _fn, _i, _j, _len, _len2, _objectToString, _ref, _ref2, _stateMachine_setState;
  ...
  Batman.SimpleHash = (function() {
    ...
    SimpleHash.prototype.equality = function(lhs, rhs) {
      if (typeof (lhs != null ? lhs.isEqual : void 0) === 'function') {
        return lhs.isEqual(rhs);
      } else if (typeof (rhs != null ? rhs.isEqual : void 0) === 'function') {
  ...
  Batman.SetSort = (function() {
    __extends(SetSort, Batman.SetObserver);
    function SetSort(base, sortKey) {
      this.base = base;
      this.sortKey = sortKey;
      SetSort.__super__.constructor.call(this, base);
      this.observedItemKeys = [this.sortKey];
      this._boundReIndex = this._reIndex.bind(this);
      this.observe('itemsWereAdded', this._boundReIndex);
      this.observe('itemsWereRemoved', this._boundReIndex);
      this.startObserving();
      this._reIndex();
    }
    ...
    SetSort.prototype.observerForItemAndKey = function() {
      return this._boundReIndex;
    };
    ...
    SetSort.prototype._reIndex = function() {
      ...
      newOrder = this.base.toArray().sort(__bind(function(a, b) {
        var valueA, valueB;
        valueA = Batman.Observable.property.call(a, this.sortKey).getValue();
        if (valueA != null) {
          valueA = valueA.valueOf();
        }
        valueB = Batman.Observable.property.call(b, this.sortKey).getValue();
        if (valueB != null) {
          valueB = valueB.valueOf();
        }
        return this.compare.call(this, valueA, valueB);
      }, this));
      this.startObservingItems.apply(this, newOrder);
  ...
  Batman.SetIndex = (function() {
    ...
    SetIndex.prototype.stopObserving = function() {
      return this._setObserver.stopObserving();
    };
    ...
    SetIndex.prototype._addItem = function(item) {
      return this._resultSetForItem(item).add(item);
    };
    SetIndex.prototype._removeItem = function(item) {
      return this._resultSetForItem(item).remove(item);
    };
  ...
  buildParams = function(prefix, obj, traditional, add) {
    ...
    if (Batman.typeOf(obj) === 'Array') {
      ...
      for (i = 0, _len3 = obj.length; i < _len3; i++) {
        v = obj[i];
        _results.push(traditional || rbracket.test(prefix) ? add(prefix, v) : buildParams(prefix + "[" + (typeof v === "object" || Batman.typeOf(v) === 'Array' ? i : "") + "]", v, traditional, add));
      }
      return _results;
    } else if (!traditional && (obj != null) && typeof obj === "object") {
      _results2 = [];
      for (name in obj) {
        _results2.push(buildParams(prefix + "[" + name + "]", obj[name], traditional, add));
      }
      return _results2;
    } else {
