(function() {
  ...
  Batman.SimpleHash = (function() {
    ...
    SimpleHash.prototype.propertyClass = Batman.ObservableProperty;
  ...
  Batman.Hash = (function() {
    ...
    Hash.prototype.propertyClass = Batman.ObservableProperty;
    Hash.accessor({
  ...
  Batman.Controller = (function() {
    ...
    Controller.prototype.render = function(options) {
      ...
      if (!options.view) {
        options.source || (options.source = helpers.underscore(this.constructor.name.replace('Controller', '')) + '/' + this._currentAction + '.html');
  ...
  Batman.StorageAdapter = (function() {
    ...
    StorageAdapter.prototype.getRecordsFromData = function(datas) {
      ...
      if (this.transformOutgoingCollectionData != null) {
        datas = this.transformOutgoingCollectionData(datas);
      }
      _results = [];
    ...
    StorageAdapter.prototype.getRecordFromData = function(data) {
      var record;
      if (this.transformIncomingRecordData != null) {
        data = this.transformIncomingRecordData(data);
      }
  ...
  Batman.RestStorage = (function() {
    ...
    RestStorage.prototype.transformIncomingRecordData = function(data) {
      if (data[this.recordJsonNamespace]) {
        return data[this.recordJsonNamespace];
      }
      return data;
    };
    RestStorage.prototype.transformOutgoingRecordData = function(record) {
      var x;
      if (this.recordJsonNamespace) {
        x = {};
        x[this.recordJsonNamespace] = record.toJSON();
        return x;
      } else {
        return record.toJSON();
      }
    };
    RestStorage.prototype.transformOutgoingCollectionData = function(data) {
    ...
    RestStorage.prototype.create = function(record, recordOptions, callback) {
      return this.optionsForRecord(record, false, function(err, options) {
        ...
        return new Batman.Request($mixin(options, {
          data: this.transformOutgoingRecordData(record),
          method: 'POST',
          success: __bind(function(data) {
            record.fromJSON(this.transformIncomingRecordData(data));
            return callback(void 0, record);
          }, this),
          error: function(err) {
            return callback(err);
      ...
      bind: function(node, key, context, renderer, only) {
        ...
        if (node.nodeName.toLowerCase() === 'input' && node.getAttribute('type') === 'checkbox') {
          ...
          return renderer.rendered(function() {
            return Batman.DOM.attrReaders.bind(node, 'value', key, context, only);
          });
        } else {
