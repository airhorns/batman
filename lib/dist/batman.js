(function() {
  ...
  Batman.Model = (function() {
    ...
    Model.load = function(options, callback) {
      ...
      this.loading(mappedRecords, options);
      this.prototype._doStorageOperation('readAll', options || {}, __bind(function(err, records) {
        var record, _i, _len;
        if (err != null) {
          ...
          for (_i = 0, _len = records.length; _i < _len; _i++) {
            record = records[_i];
            mappedRecords.add(this._mapIdentity(record));
          }
          mappedRecords.fire('loaded', options);
          this.loaded(mappedRecords, options);
      ...
      if (id = this.model.get(this.association.localKey)) {
        ...
        if (relatedRecord != null) {
          ...
          return this.association.getRelatedModel().load(loadOptions, __bind(function(error, loadedRecords) {
            ...
            if (!loadedRecords || loadedRecords.length < 1) {
              return callback(new Error("Couldn't find related record!"), void 0);
            } else {
              this.set('loaded', true);
              return callback(void 0, loadedRecords.get('first'));
            }
          }, this));
        }
      }
      ...
      if (setInAttributes = self.getFromAttributes(this)) {
        ...
        Batman.Property.withoutTracking(__bind(function() {
          if (self.options.autoload && !this.isNew() && !relatedRecords.loaded) {
            return relatedRecords.load(function(error) {
              if (error) {
                throw error;
              }
            });
