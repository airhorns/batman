(function() {
  ...
  Batman.DOM.StyleHandler = (function() {
    function StyleHandler(node, key, context, only) {
      this.node = node;
      this.key = key;
      ...
      context.bind(this.node, this.key, this.dataChange, this.nodeChange, only);
    }
    StyleHandler.prototype.dataChange = function(value) {
      var cssKey, cssValue, _results;
      if (!value) {
      ...
      if (value instanceof Batman.Hash) {
        ...
        _results = [];
        for (cssKey in value) {
          if (!__hasProp.call(value, cssKey)) continue;
          cssValue = value[cssKey];
          _results.push(this.node.style[cssKey] = cssValue);
        }
        return _results;
      }
    };
    StyleHandler.prototype.nodeChange = function(node, subContext) {
      this.node = node;
      return subContext.set(this.key, this.node.style.cssText);
    };
    StyleHandler.prototype.onItemsAdded = function(newKey) {
      return this.node.style[newKey] = this.styleHash.get(newKey);
    };
    StyleHandler.prototype.onItemsRemoved = function(oldKey) {
