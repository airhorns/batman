(function() {
  ...
  Batman.Model = (function() {
    ...
    Model.prototype.validate = function(callback, errors) {
      ...
      finish = __bind(function() {
        this.validated();
        this[oldState]();
        return typeof callback === "function" ? callback(errors.length === 0, errors) : void 0;
      }, this);
    ...
    Model.prototype.isValid = function(callback) {
      var errors;
      errors = new Batman.Set;
      return this.validate(callback, errors);
    };
    return Model;
  })();
  ...
  Batman.LocalStorage = (function() {
    ...
    function LocalStorage() {
      ...
      for (k in _ref2) {
        ...
        if (matches = this.key_re.exec(k)) {
          this.nextId = Math.max(this.nextId, parseInt(matches[1], 10) + 1);
        }
      }
      return;
    }
    LocalStorage.prototype.getRecordFromData = function(data) {
      ...
      this.nextId = Math.max(this.nextId, record.get('id') + 1);
      return record;
    };
    ...
    LocalStorage.prototype.readAll = function(_, options, callback) {
      var data, k, keyMatches, match, records, storageKey, v, _name, _ref2;
      records = [];
      _ref2 = this.storage;
      for (storageKey in _ref2) {
        data = _ref2[storageKey];
        if (keyMatches = this.key_re.exec(storageKey)) {
          match = true;
          data = JSON.parse(data);
          data[_name = this.model.primaryKey] || (data[_name] = keyMatches[1]);
          for (k in options) {
            v = options[k];
            if (data[k] !== v) {
              match = false;
