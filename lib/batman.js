(function() {
  ...
  Batman.DOM.Style = (function() {
    function Style(node, key, context) {
      ...
      context.bind(node, key, this.dataChange, this.nodeChange);
    }
    Style.prototype.dataChange = function(value) {
      var key, keyValue, keypathContext, keypathValue, _ref, _results;
      if (!value) {
        return this.node.setAttribute('style', '');
      }
      if (typeof value === 'string' && (this.boundValueType = 'string')) {
        return this.node.setAttribute('style', value);
      }
      if (value instanceof Batman.Hash && (this.boundValueType = 'batman.hash')) {
        ...
        for (key in value) {
          ...
          keyValue = value[key];
          _ref = this.context.findKey(keyValue), keypathValue = _ref[0], keypathContext = _ref[1];
          _results.push(this.node.style[key] = keypathValue ? (this.bindSingleAttribute(key, keyValue), keypathValue) : keyValue);
        }
        return _results;
      }
    };
    Style.prototype.nodeChange = function(node, subContext) {
      var hash, key, keypathValue, object, value, _results;
      switch (this.boundValueType) {
        case 'string':
          return subContext.set(this.key, this.node.style.cssText);
        case 'batman.hash':
          hash = subContext.get(this.key);
          return hash.keys().forEach(__bind(function(key) {
            return hash.set(key, this.node.style[key]);
          }, this));
        case 'object':
          object = subContext.get(this.key);
          _results = [];
          for (key in object) {
            if (!__hasProp.call(object, key)) continue;
            value = object[key];
            keypathValue = subContext.get(value);
            _results.push(keypathValue ? subContext.set(value, this.node.style[key]) : object[key] = this.node.style[key]);
          }
          return _results;
      }
    };
    Style.prototype.onItemsAdded = function(newKey) {
      return this.node.style[newKey] = this.styleHash.get(newKey);
    };
    Style.prototype.onItemsRemoved = function(oldKey) {
      return this.node.style[oldKey] = '';
    };
    Style.prototype.bindSingleAttribute = function(attr, keypath) {
      var dataChange, nodeChange;
      dataChange = function(value) {
        return this.node.style[attr] = value;
      };
      nodeChange = function(node, subContext) {
        return subContext.set(keypath, node.style[attr]);
      };
      return this.context.bind(this.node, keypath, dataChange, nodeChange);
    };
    return Style;
  })();
