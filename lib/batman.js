(function() {
  ...
  Batman.Set = (function() {
    ...
    _ref2 = ['has', 'forEach', 'isEmpty', 'toArray'];
    for (_j = 0, _len2 = _ref2.length; _j < _len2; _j++) {
      k = _ref2[_j];
  ...
  Batman.SetIndex = (function() {
    ...
    function SetIndex(base, key) {
      ...
      this._itemsWereAddedCallback = __bind(function() {
      ...
      this._itemsWereRemovedCallback = __bind(function() {
        var item, items, _j, _len2, _results;
        items = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
        _results = [];
        for (_j = 0, _len2 = items.length; _j < _len2; _j++) {
      ...
      this.base.observe('itemsWereAdded', this._itemsWereAddedCallback);
      this.base.observe('itemsWereRemoved', this._itemsWereRemovedCallback);
      this.base.forEach(this._addItem.bind(this));
    }
    SetIndex.accessor({
      get: function(key) {
        return this._getOrInitialize(this._storage, key, function() {
          return new Batman.Set;
        });
      }
    });
    SetIndex.prototype.stopUpdating = function() {
      this.base.forEach(__bind(function(item) {
        return this._forgetItem(item);
      }, this));
      this.base.forget('itemsWereAdded', this._itemsWereAddedCallback);
      return this.base.forget('itemsWereRemoved', this._itemsWereRemovedCallback);
    };
    SetIndex.prototype._observerForItem = function(item) {
      return this._getOrInitialize(this._itemObservers, item, __bind(function() {
        return __bind(function(newValue, oldValue) {
          this.get(oldValue).remove(item);
          return this.get(newValue).add(item);
        }, this);
      }, this));
    };
    SetIndex.prototype._getOrInitialize = function(hash, key, lazyDefault) {
      var set;
      set = hash.get(key);
      if (!set) {
        set = lazyDefault();
    ...
    SetIndex.prototype._addItem = function(item) {
      this._resultSetForItem(item).add(item);
      return item.observe(this.key, this._observerForItem(item));
    };
    SetIndex.prototype._forgetItem = function(item) {
      item.forget(this.key, this._observerForItem(item));
      return this._itemObservers.unset(item);
    };
    SetIndex.prototype._removeItem = function(item) {
      this._resultSetForItem(item).remove(item);
      return this._forgetItem(item);
    };
    SetIndex.prototype._resultSetForItem = function(item) {
      return this.get(Batman.Keypath.forBaseAndKey(item, this.key).getValue());
    };
    return SetIndex;
  })();
  ...
  Batman.Model = (function() {
    ...
    Model.prototype.load = function(callback) {
      ...
      return this._doStorageOperation('read', {}, __bind(function(err, record) {
        ...
        return typeof callback === "function" ? callback(err, this.constructor._mapIdentities([record])[0]) : void 0;
      }, this));
    };
  ...
  Batman.RestStorage = (function() {
    ...
    RestStorage.prototype.optionsForCollection = function(recordsOptions, callback) {
      ...
      if (!url) {
        ...
        return callback.call(this, void 0, $mixin({}, this.defaultOptions, {
          url: url,
          data: JSON.stringify(recordsOptions)
        }));
      }
    };
  ...
  Batman.View = (function() {
    ...
    View.observeAll('node', function(node) {
      ...
      if (node) {
        ...
        this._renderer.rendered(__bind(function() {
  ...
  Batman.Renderer = (function() {
    ...
    Renderer.prototype.parseNode = function(node) {
      var attr, bindings, name, nextNode, readerArgs, result, skipChildren, varIndex, _base, _base2, _j, _len2, _name, _name2, _ref2;
      ...
      if (node.getAttribute) {
        ...
        for (_j = 0, _len2 = _ref2.length; _j < _len2; _j++) {
          readerArgs = _ref2[_j];
          result = readerArgs.length === 2 ? typeof (_base = Batman.DOM.readers)[_name = readerArgs[0]] === "function" ? _base[_name](node, readerArgs[1], this.context, this) : void 0 : typeof (_base2 = Batman.DOM.attrReaders)[_name2 = readerArgs[0]] === "function" ? _base2[_name2](node, readerArgs[1], readerArgs[2], this.context, this) : void 0;
          if (result === false) {
            skipChildren = true;
            break;
          }
    ...
    readers: {
      bind: function(node, key, context) {
        ...
        if (node.nodeName.toLowerCase() === 'input' && node.getAttribute('type') === 'checkbox') {
          ...
          return view._renderer.rendered(function() {
