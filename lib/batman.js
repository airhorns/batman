(function() {
    ...
    Batman.Binding.prototype = {
        ...
        isAllowed: function() {
            return typeof this.preventCount !== 'undefined' && this.preventCount == 0;
        },
            ...
            if (Array.isArray(value)) {
                ...
                var keys = this._keysFromValue,
                    binder = function(key) {
                        ...
                        var newBinding = Batman.Binding(f, f);
                        newBinding.observeDependencies();
                        return newBinding;
                    }.bind(this);
                
                for (var key in value) {
                    if (((key in this) && Array.indexOf(keys, key) === -1 && key !== 'name') || key.substr(0,1) === '_')
                        continue;
                    
                    var newValue = value[key],
                        newKey = key === 'name' ? '$name' : key; // FIXME: name is a reserved key on Function. Not sure if this is the right symbol
                                                                    // to replace it with, or if it should even be replaced. If it is, make sure it's well documented!
                    
                    // if (newValue && newValue.isBinding)
                        // this[newKey] = binder(key);
                    // else
        ...
        validate: function(value, oldValue) {
            ...
            return Batman.execute(this._validators, value, oldValue);
        }
    };
