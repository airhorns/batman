(function() {
    ...
    var require = function(files, extensionOrCallback, callback) {
        ...
        var extension = 'js';
        if (typeof extensionOrCallback === 'string')
        ...
        var requestCallback = function(data, request) {
            var i = files.length;
            while (i--)
                if (!requiredFiles[files[i]].completed())
                    return;
            
            for (var i = -1, count = files.length; ++i < count;)
                (new Function(requiredFiles[files[i]].data()))();
            
            Batman.execute(callback);
        };
        
        var i = files.length, request;
        while (i--) {
            var file = files[i];
            if (requiredFiles[file]) {
                if (!requiredFiles[file].completed())
                    request = requiredFiles[file];
            }
            else {
                var url;
                if (file.substr(file.length - extension.length) === extension)
                    ...
                    url = (Batman.request.base || '') + file + extension;
                
                request = requiredFiles[file] = Batman.request(url);
            }
    ...
    Batman.Binding = function(getter, setter) {
        var binding = function(value) {
            if (arguments.length > 0) {
                ...
                binding.value = binding.validate(value, oldValue);
    ...
    Batman.Binding.prototype = {
        ...
        validator: function(func, validateImmediately) {
            if (!this._validators)
                this._validators = [];
    ...
    Batman.event = function(func, fireOnce) {
        var emitterFunc = function() {
            ...
            if (typeof func === 'function')
                result = func.apply(emitter.context || this, arguments);
            
            if (result !== false)
                emitter.fire(result);
        }, emitter = Batman.Binding(emitterFunc, emitterFunc);
        
        emitter.isEvent = true;
        emitter.identifier = 'event';
        
        emitter.configure = function(object, key) {
            delete emitter._configureOnMixin;
            
            emitter.context = object;
            emitter.identifier = 'event: ' + key;
            
            object['on' + key.toLowerCase()] = emitter.on.bind(emitter);
            
            return emitter;
        };
        
        emitter.copy = function(object, key) {
            return Batman.event(func, fireOnce);
        };
    ...
    Batman.request = function(url, callback, options) {
        var request = Batman.Request({url: url}, options);
        if (callback)
    ...
    Batman.Request = Batman.Mixin({
        isRequest: true,
        completed: Batman.binding(false),
        active: Batman.binding(false),
        
        url: Batman.binding(''),
        method: Batman.binding('GET'),
        data: Batman.binding(''),
        
        complete: Batman.event(function(data) {
            this.data(data);
            this.completed(true);
        }, true),
        
        fire: function() {
            var request = window.XMLHttpRequest && (window.location.protocol !== "file:" || !window.ActiveXObject) ? new window.XMLHttpRequest() : new window.ActiveXObject("Microsoft.XMLHTTP");
            if (!request)
                return;
            ...
            request.onreadystatechange = function() {
                ...
                this.complete(request.responseText);
            }.bind(this);
