(function() {
  ...
  Batman.LocalStorage = (function() {
    ...
    LocalStorage.prototype._storageEntriesMatching = function(proto, options) {
      ...
      records = [];
      this._forAllStorageEntries(function(storageKey, env) {
        var data, error, keyMatches, _ref;
        if (keyMatches = re.exec(storageKey)) {
          _ref = this._jsonToAttributes(env), error = _ref[0], data = _ref[1];
          if (error != null) {
            return [error, []];
          }
          data[proto.constructor.primaryKey] = keyMatches[1];
          if (this._dataMatches(options, env)) {
    ...
    LocalStorage.prototype._dataMatches = function(conditions, env) {
      var k, match, v;
      match = true;
      for (k in conditions) {
        v = conditions[k];
    ...
    LocalStorage.prototype.after('read', LocalStorage.skipIfError(function(env, next) {
      ...
      if (typeof env.recordAttributes === 'string') {
        _ref = this._jsonToAttributes(env.recordAttributes), error = _ref[0], env.recordAttributes = _ref[1];
        if (error != null) {
          env.error = error;
          return next();
        }
  ...
  Batman.RestStorage = (function() {
    ...
    RestStorage.prototype.request = function(options, callback) {
      if (options.error != null) {
        return this.runAfterFilter(options.action, options, callback);
      }
      options = $mixin(options, {
        success: __bind(function(data) {
          ...
          return this.runAfterFilter(env.action, env, callback);
        }, this),
    ...
    RestStorage.prototype.before('readAll', RestStorage.skipIfError(function(env, next) {
      try {
        env.url = this.urlForCollection(env.proto.constructor, env);
      } catch (error) {
        env.error = error;
      }
      return next();
    ...
    RestStorage.prototype.before('create', 'update', RestStorage.skipIfError(function(env, next) {
      ...
      if (this.serializeAsForm) {
        env.contentType = this.constructor.PostBodyContentType;
      } else {
        env.data = JSON.stringify(env.data);
        env.contentType = this.constructor.JSONContentType;
      }
      return next();
    }));
    RestStorage.prototype.after('create', 'read', 'update', RestStorage.skipIfError(function(env, next) {
      ...
      if (typeof env.data === 'string') {
        _ref = this._jsonToAttributes(env.env), error = _ref[0], json = _ref[1];
        env.error = error;
        return next();
      } else {
        json = env.data;
    ...
    _fn = __bind(function(key) {
      return this.prototype[key] = function(record, options, callback) {
        return this.runBeforeFilter(key, {
          ...
          env.method = this.constructor.HTTPMethods[key];
          return this.request(env, callback);
        });
      };
    }, RestStorage);
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    ...
    RestStorage.prototype.readAll = function(proto, options, callback) {
      return this.runBeforeFilter('readAll', {
        ...
        env.method = this.constructor.HTTPMethods['readAll'];
        return this.request(env, callback);
      });
    };
    return RestStorage;
