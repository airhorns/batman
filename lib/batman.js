(function() {
    ...
    Batman.event = function(func, fireOnce) {
        var handler = function(observer) {
            ...
            if (typeof func === 'function')
                result = func.apply(handler.context || handler, arguments);
            
            result = result || Array.toArray(arguments);
    ...
    Batman.Event = Batman.Mixin({
        isEvent: true,
        _configureOnMixin: true
    }, Batman.Binding.inherit(['fire', 'observe', 'forget', 'observeForever', 'configure', 'allow', 'prevent', 'allowed']));
