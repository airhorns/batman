(function() {
  ...
  Batman.Association = (function() {
    var AssociationProxy, AssociationSet, AssociationSetIndex;
    ...
    AssociationProxy = (function() {
      ...
      AssociationProxy.prototype.load = function(model, callback) {
        if (!callback) {
          callback = model;
        } else {
          this.model = model;
        }
        this.association.fetch(callback, this.model, this);
      ...
      AssociationProxy.accessor('target', {
        get: function() {
          var id, relatedKey;
          relatedKey = this.association.getRelatedKey();
          if (id = this.model.get(relatedKey)) {
            return this.association.getRelatedModel().get('loaded').indexedBy('id').get(id).toArray()[0];
          }
        },
        set: function(v) {
          return v;
    ...
    AssociationSetIndex = (function() {
      ...
      AssociationSetIndex.prototype._resultSetForKey = function(key) {
        return this._storage.getOrSet(key, __bind(function() {
          return new AssociationSet(key, this.association);
        }, this));
      };
      return AssociationSetIndex;
    })();
    Association.prototype.setIndex = function() {
      this.index || (this.index = new AssociationSetIndex(this));
      return this.index;
    };
    Association.prototype.getAccessor = function(self, model, label) {
      var proxy, recordInAttributes, _base, _base2, _name;
      if (recordInAttributes = self.getFromAttributes(this)) {
        return recordInAttributes;
      }
      if (self.getRelatedModel()) {
        (_base = this._batman).associations || (_base.associations = {});
        proxy = (_base2 = this._batman.associations)[_name = this.label] || (_base2[_name] = new AssociationProxy(self, this));
        if (!proxy.get('loaded') && self.options.autoload) {
          proxy.load(this, function(err, relatedRecord) {
            return proxy.set('target', relatedRecord);
          });
        }
        return proxy;
      }
  ...
  Batman.Association.belongsTo = (function() {
    ...
    belongsTo.prototype.fetch = function(callback, model, proxy) {
      var loadedRecords, relatedID;
      if (relatedID = model.get("" + this.label + "_id")) {
        loadedRecords = this.setIndex().get(relatedID);
        if (!loadedRecords.isEmpty()) {
          return callback(void 0, loadedRecords.toArray()[0]);
        } else {
          return this.getRelatedModel().find(relatedID, __bind(function(error, loadedRecord) {
            if (error) {
              throw error;
            }
            if (loadedRecord) {
    ...
    belongsTo.prototype.getRelatedKey = function() {
  ...
  Batman.Association.hasOne = (function() {
    ...
    hasOne.prototype.getRelatedKey = function() {
      return "id";
    };
    hasOne.prototype.fetch = function(callback, model, proxy) {
      var id, loadOptions, relatedRecords;
      if (id = model.get('id')) {
        relatedRecords = this.setIndex().get(id);
        if (!relatedRecords.isEmpty()) {
          proxy.set('loaded', true);
          return callback(void 0, relatedRecords.toArray()[0]);
        } else {
          loadOptions = {};
          loadOptions[this.foreignKey] = id;
          return this.getRelatedModel().load(loadOptions, __bind(function(error, loadedRecords) {
            if (error) {
              throw error;
            }
