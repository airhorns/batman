(function() {
    ...
    event: $block(function(key, context, callback) {
      ...
      f = function(observer) {
        ...
        if (typeof observer === 'function') {
          ...
          if (value !== false) {
            f._firedArgs = value != null ? (_ref2 = [value]).concat.apply(_ref2, arguments) : arguments.length === 0 ? [] : Array.prototype.slice.call(arguments);
  ...
  */
  Batman.Set = (function() {
    ...
    Set.prototype.add = Set.event(function(item) {
      this._storage.set(item, true);
      this.set('length', this.length + 1);
      return item;
    });
    Set.prototype.remove = Set.event(function(item) {
      this._storage.remove(item);
      return this.set('length', this.length - 1);
    });
  ...
  */
  ...
  $mixin(Batman, {
    ...
    route: $block(function(pattern, callback) {
      var f;
      f = function(params) {
        var array, context, key, match, namedArguments, regexp, value;
        ...
        if (context && context.dispatch) {
          context.dispatch.apply(context, [f].concat(__slice.call(args)));
        } else {
    ...
    Renderer.prototype.parseNode = function(node) {
      ...
      if (node.getAttribute) {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          ...
          result = (index = name.indexOf('-')) === -1 ? typeof (_base = Batman.DOM.readers)[name] === "function" ? _base[name](node, attr.value, contexts) : void 0 : typeof (_base2 = Batman.DOM.attrReaders)[_name = name.substr(0, index)] === "function" ? _base2[_name](node, name.substr(index + 1), attr.value, contexts) : void 0;
          if (result === false) {
            skipChildren = true;
            break;
          }
  ...
  Batman.DOM = {
    readers: {
      bind: function(node, key, contexts) {
        ...
        return context.observe(key, true, observer = function(value) {
          if (shouldSet) {
    ...
    attrReaders: {
      ...
      foreach: function(node, iteratorName, key, contexts) {
        var add, collection, context, contextsClone, nodeMap, prototype, remove;
