(function() {
      ...
      if (pairs = this._storage[hashKey]) {
        for (index = 0, _len = pairs.length; index < _len; index++) {
          ...
          if (this.equality(obj, key)) {
            ...
            if (!pairs.length) delete this._storage[hashKey];
            this.length--;
            return pair[0][1];
          }
        }
  ...
  Batman.SimpleSet = (function() {
    ...
    SimpleSet.prototype.add = function() {
      ...
      for (_i = 0, _len = items.length; _i < _len; _i++) {
        item = items[_i];
        if (!(!this._storage.hasKey(item))) continue;
  ...
  Batman.Dispatcher = (function() {
    ...
    ControllerDirectory = (function() {
      ...
      ControllerDirectory.accessor('app', Batman.Property.defaultAccessor);

      ControllerDirectory.accessor(function(key) {
        return this.get("app." + (helpers.capitalize(key)) + "Controller.sharedController");
      });
  ...
  Batman.Model = (function() {
    ...
    Model.encode = function() {
      ...
      if (typeof encoder === 'undefined') encoder = this.defaultEncoder.encode;
      if (typeof decoder === 'undefined') decoder = this.defaultEncoder.decode;
      _results = [];
      for (_j = 0, _len = keys.length; _j < _len; _j++) {
        key = keys[_j];
        if (encoder) this.prototype._batman.encoders.set(key, encoder);
        if (decoder) {
          _results.push(this.prototype._batman.decoders.set(key, decoder));
        } else {
    ...
    Model.prototype.load = function(callback) {
      ...
      this.loading();
      return this._doStorageOperation('read', {}, function(err, record) {
        if (!err) {
          _this.loaded();
    ...
    Model.prototype.save = function(callback) {
      ...
      return this.validate(function(isValid, errors) {
        ...
        return _this._doStorageOperation((creating ? 'create' : 'update'), {}, function(err, record) {
          var _ref5, _ref6;
          if (!err) {
    ...
    Model.prototype.destroy = function(callback) {
      var _this = this;
      this.destroying();
      return this._doStorageOperation('destroy', {}, function(err, record) {
        if (!err) {
  ...
  Batman.AssociationProxy = (function() {
    ...
    AssociationProxy.prototype.toJSON = function() {
      if (this.get('loaded')) return this.get('target').toJSON();
    };

    AssociationProxy.prototype.load = function(callback) {
      var _this = this;
      this.fetch(function(err, relation) {
        if (!err) _this.set('target', relation);
    ...
    BelongsToProxy.prototype.fetch = function(callback) {
      ...
      if (relatedID = this.model.get(this.association.localKey)) {
        ...
        if (loadedRecord != null) {
          ...
          return this.association.getRelatedModel().find(relatedID, function(error, loadedRecord) {
            if (error) throw error;
            if (loadedRecord) _this.set('loaded', true);
            return callback(void 0, loadedRecord);
          });
        }
      }
  ...
  Batman.AssociationSet = (function() {
    ...
    AssociationSet.prototype.load = function(callback) {
      ...
      return this.association.getRelatedModel().load(loadOptions, function(err, records) {
  ...
  Batman.PluralAssociation = (function() {
    ...
    PluralAssociation.prototype.setForRecord = function(record) {
      ...
      return Batman.Property.withoutTracking(function() {
        var id;
        if (id = record.get(_this.localKey)) {
  ...
  Batman.BelongsToAssociation = (function() {
    ...
    BelongsToAssociation.prototype.apply = function(base) {
      ...
      if (model = base.get(this.label)) {
        ...
        if (foreignValue !== void 0) return base.set(this.localKey, foreignValue);
      }
    };
      ...
      if (!baseSaveError) {
        if (relations = this.getFromAttributes(base)) {
          relations.forEach(function(model) {
            return model.set(_this.foreignKey, base.get(_this.localKey));
          });
        }
  ...
  Batman.View = (function() {
    ...
    View.prototype.render = function(node) {
      ...
      if ((_ref2 = this._renderer) != null) _ref2.forgetAll();
      if (node) {
  ...
  Batman.Renderer = (function() {
    ...
    function Renderer(node, callback, context, view) {
      ...
      if (view) this.context = this.context.descend(view);
        ...
        bindings = (function() {
          ...
          for (_k = 0, _len4 = _ref3.length; _k < _len4; _k++) {
            ...
            if (~(varIndex = name.indexOf('-'))) {
              _results.push([name.substr(0, varIndex), name.substr(varIndex + 1), attr.value]);
            } else {
              _results.push([name, attr.value]);
            }
          }
          return _results;
        })();
  ...
  Batman.RenderContext = (function() {
    ...
    RenderContext.start = function(context) {
      ...
      if (Batman.currentApp) node = node.descend(Batman.currentApp);
      if (context) node = node.descend(context);
      return node;
    };
  ...
  Batman.DOM = {
    ...
    contentFor: function(name, node, _replaceContent, _yieldChildren) {
      ...
      yieldFn = function(yieldingNode) {
        ...
        if (_yieldChildren) {
          ...
          if (view != null) view.viewWillAppear(child);
          $appendChild(yieldingNode, node, true);
          if (view != null) view.viewDidAppear(child);
        }
      ...
      if (yieldingNode) return Batman.DOM.yield(name, yieldingNode);
    },
    ...
    appendChild: $appendChild = function() {
      ...
      view = Batman.data(child, 'view');
      if (view != null) {
        if (typeof view.viewWillAppear === "function") view.viewWillAppear(child);
      }
    ...
    addEventListener: $addEventListener = function(node, eventName, callback) {
      ...
      if (!listeners[eventName]) listeners[eventName] = new Batman.Set;
    ...
    removeEventListener: $removeEventListener = function(node, eventName, callback) {
      ...
      if (listeners = Batman._data(node, 'listeners')) {
        if (eventListeners = listeners[eventName]) eventListeners.remove(callback);
      }
      if ($hasAddEventListener) {
      ...
      if (filters.length) {
        ...
        while (filterString = filters.shift()) {
          ...
          if (filter = Batman.Filters[filterName]) {
            ...
            if (args) {
              try {
                _results.push(this.filterArguments.push(this.parseSegment(args)));
              } catch (e) {
                _results.push(developer.error("Bad filter arguments \"" + args + "\"!"));
              }
            } else {
              _results.push(this.filterArguments.push([]));
            }
          } else {
            _results.push(developer.error("Unrecognized filter '" + filterName + "' in key \"" + this.keyPath + "\"!"));
          }
        }
        return _results;
      }
  ...
  Batman.DOM.RouteBinding = (function() {
    ...
    RouteBinding.prototype.bind = function() {
      ...
      return Batman.DOM.events.click(this.node, function() {
        var params;
        params = _this.get('filteredValue');
    ...
    RouteBinding.prototype.dataChange = function(value) {
      var path, _ref2;
      if (value != null) {
        path = value.isNamedRouteQuery ? value.get('path') : (_ref2 = this.get('dispatcher')) != null ? _ref2.pathFromParams(value) : void 0;
      }
      if (this.onATag) {
        if (path != null) {
  ...
  Batman.DOM.ViewBinding = (function() {
    ...
    ViewBinding.prototype.dataChange = function(viewClassOrInstance) {
      ...
      this.view.on('ready', function() {
        ...
        if (typeof (_base2 = _this.view).viewWillAppear === "function") {
          _base2.viewWillAppear(_this.node);
        }
        _this.renderer.allowAndFire('rendered');
        return typeof (_base3 = _this.view).viewDidAppear === "function" ? _base3.viewDidAppear(_this.node) : void 0;
      });
      return this.die();
    };
  ...
  Batman.DOM.FormBinding = (function() {
    ...
    FormBinding.prototype.setupErrorsList = function() {
      if (this.errorsListNode = this.get('node').querySelector(this.get('errorsListSelector'))) {
