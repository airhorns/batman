(function() {
  ...
  Batman.Dispatcher = (function() {
    ...
    Dispatcher.prototype.pathFromParams = function(params) {
      if ($typeOf(params) === 'String') {
        return Batman.Navigation.normalizePath(params);
      } else {
  ...
  Batman.HistoryManager = (function() {
    ...
    HistoryManager.prototype.handleLocation = function(location) {
      var path;
    ...
    HistoryManager.prototype.handleCurrentLocation = function() {
      return this.handleLocation(window.location);
    };
    HistoryManager.prototype.dispatch = function(params) {
      return this.cachedPath = this.app.dispatcher.dispatch(params);
    };
    HistoryManager.prototype.push = function(params) {
      var path;
      path = this.dispatch(params);
      this.pushState(null, '', path);
      return path;
    };
    HistoryManager.prototype.replace = function(params) {
      var path;
      path = this.dispatch(params);
      this.replaceState(null, '', path);
      return path;
    };
    HistoryManager.prototype.redirect = HistoryManager.prototype.push;
    HistoryManager.prototype.normalizePath = Batman.Navigation.normalizePath;
  ...
  Batman.StateHistory = (function() {
    ...
    StateHistory.prototype.pushState = function(stateObject, title, path) {
      return window.history.pushState(stateObject, title, this.linkTo(path));
    };
    StateHistory.prototype.replaceState = function(stateObject, title, path) {
      return window.history.replaceState(stateObject, title, this.linkTo(path));
    };
    StateHistory.prototype.linkTo = function(url) {
      return this.normalizePath(Batman.pathPrefix, url);
    };
    StateHistory.prototype.pathFromLocation = function(location) {
      var fullPath, prefixPattern;
      fullPath = "" + (location.pathname || '') + (location.search || '');
      prefixPattern = new RegExp("^" + (this.normalizePath(Batman.pathPrefix)));
      return this.normalizePath(fullPath.replace(prefixPattern, ''));
    };
    StateHistory.prototype.handleLocation = function(location) {
      var hashbangPath, path;
      path = this.pathFromLocation(location);
      if (path === '/' && (hashbangPath = Batman.HashHistory.prototype.pathFromLocation(location)) !== '/') {
        return this.replace(hashbangPath);
      } else {
        return StateHistory.__super__.handleLocation.apply(this, arguments);
      }
    };
    return StateHistory;
  })();
  Batman.HashHistory = (function() {
    ...
    HashHistory.prototype.pushState = function(stateObject, title, path) {
      return window.location.hash = this.linkTo(path);
    };
    HashHistory.prototype.replaceState = function(stateObject, title, path) {
      var loc;
      loc = window.location;
      return loc.replace("" + loc.pathname + loc.search + (this.linkTo(path)));
    };
    HashHistory.prototype.linkTo = function(url) {
      return this.HASH_PREFIX + url;
    };
    HashHistory.prototype.pathFromLocation = function(location) {
      var hash;
      hash = location.hash;
    ...
    HashHistory.prototype.handleLocation = function(location) {
      var realPath;
      realPath = Batman.StateHistory.prototype.pathFromLocation(location);
      if (realPath === '/') {
        return HashHistory.__super__.handleLocation.apply(this, arguments);
      } else {
        return location.replace(this.normalizePath("" + Batman.pathPrefix + (this.linkTo(realPath))));
      }
    };
    return HashHistory;
  })();
  ...
  Batman.View = (function() {
    ...
    View.accessor('html', function() {
      ...
      path = Batman.Navigation.normalizePath(this.prefix, source);
    ...
    readers: {
      ...
      route: function(node, key, context) {
        ...
        if (node.nodeName.toUpperCase() === 'A') {
          node.href = Batman.HistoryManager.defaultClass.prototype.linkTo(url);
        }
      ...
      defineview: function(node, name, context, renderer) {
        ...
        Batman.View.sourceCache.set(Batman.Navigation.normalizePath(Batman.View.prototype.prefix, name), node.innerHTML);
        return false;
      },
