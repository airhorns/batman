(function() {
  ...
  Batman.Association = (function() {
    ...
    Association.prototype.encodeModelIntoObject = function(model, obj) {
      var relation;
      if (relation = model.get(this.label)) {
        obj[this.label] = relation.toJSON();
        return delete obj["" + this.label + "_id"];
      }
    };
    Association.prototype.decodeObjectIntoModel = function(model, obj, data) {
      var json, record, relatedModel, relatedModelName, _ref;
      if (json = data[this.label]) {
        relatedModelName = helpers.camelize(helpers.singularize(this.label));
        if (relatedModel = (_ref = Batman.currentApp) != null ? _ref[relatedModelName] : void 0) {
    ...
    BatmanAssociationCollection = (function() {
      ...
      BatmanAssociationCollection.prototype.encodeModelIntoObject = function(model, obj) {
        ...
        if (!this._encodingRelation) {
          ...
          modelName = $functionName(model.constructor);
          if (modelHash = this.storage.get(modelName)) {
            modelHash.forEach(function(type, typeHash) {
              return typeHash.forEach(function(association, label) {
  ...
  Batman.Association.belongsTo = (function() {
    ...
    belongsTo.prototype.getAccessor = function(model, label) {
      var loadedRecord, relatedID, relatedModel, relatedModelName, relatedRecord, _ref, _ref2;
      ...
      if (!(relatedModel = (_ref2 = Batman.currentApp) != null ? _ref2[relatedModelName] : void 0)) {
        return;
      }
  ...
  Batman.Association.hasOne = (function() {
    ...
    hasOne.prototype.getAccessor = function(model, label) {
      var existingInstance, id, loadOptions, loadedRecord, modelName, relatedModel, relatedModelName, relatedRecords, _ref;
      if (this.amSetting) {
        return;
      }
    ...
    hasOne.prototype.clearRelation = function(base) {
      var baseName, relatedModel, relatedModelName, _ref;
  ...
  Batman.Association.hasMany = (function() {
    ...
    hasMany.prototype.getAccessor = function(model, label) {
      var existingInstance, id, loadOptions, loadedRecords, modelName, recordInAttributes, relatedModel, relatedModelName, relatedRecords, _ref, _ref2;
      if (this.amSetting) {
        return;
      }
      existingInstance = Batman.Model.defaultAccessor.get.call(this, label);
      ...
      if (!relatedRecords.isEmpty()) {
      ...
      } else if (recordInAttributes = (_ref2 = this._batman.attributes) != null ? _ref2[label] : void 0) {
        ...
        relatedModel.load(loadOptions, __bind(function(error, records) {
          ...
          for (_i = 0, _len = records.length; _i < _len; _i++) {
            record = records[_i];
            _results.push(loadedRecords.add(record));
          }
          return _results;
        }, this));
    ...
    hasMany.prototype.decodeObjectIntoModel = function(model, obj, data) {
      var id, idRegex, json, record, relatedModel, relations, storageKey, _, _ref, _ref2;
      if (json = data[this.label]) {
        relations = new Batman.Set;
        if (relatedModel = (_ref = Batman.currentApp) != null ? _ref[helpers.camelize(helpers.singularize(this.label))] : void 0) {
          idRegex = new RegExp("^" + ($functionName(relatedModel).toLowerCase()) + "s(\\d+)$");
          for (storageKey in json) {
            if (!__hasProp.call(json, storageKey)) continue;
            obj = json[storageKey];
