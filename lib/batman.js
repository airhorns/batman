...
(function() {
    ...
    Batman.requirePath = function(file, extension) {
        ...
        if (file.substr(file.length - extension.length) === extension)
            return Batman.LIB_PATH + file;
        else
            return (Batman.REQUIRE_PATH || '') + file;
    };
    ...
    if (!Batman.Model)
        require(Batman.requirePath('batman.mvc.js'), Batman.ready);
    else
        Batman.ready();
    
})();

// globalize the exports in the browser
