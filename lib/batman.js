(function() {
  ...
  Batman.Model = (function() {
    ...
    Model.belongsTo = function(label) {
      return new Batman.Association.belongsTo(this, label);
    };
    Model.hasOne = function(label) {
      return new Batman.Association.hasOne(this, label);
    };
    Model.hasMany = function(label) {
      return new Batman.Association.hasMany(this, label);
    };
    Model.accessor('id', {
      get: function() {
        var pk;
    ...
    Model.prototype.toJSON = function() {
      ...
      if (!(!encoders || encoders.isEmpty())) {
        encoders.forEach(__bind(function(key, encoder) {
          var encodedVal, val;
          if (Batman.Association.Collection.hasAssociationOn(this, key)) {
            return;
          }
          val = this.get(key);
    ...
    Model.prototype.save = function(callback) {
      ...
      return this.validate(__bind(function(isValid, errors) {
        var associations, belongsTo, creating, hasMany, hasOne, modelSaved;
        if (!isValid) {
          if (typeof callback === "function") {
            callback(errors);
          }
    ...
    Model.prototype.destroy = function(callback) {
      ...
      return this._doStorageOperation('destroy', {}, __bind(function(err, record) {
        Batman.Association.Collection.clearModelRelations(this);
        if (!err) {
  ...
  Batman.Association = (function() {
    function Association(model, label) {
      ...
      getAccessor = function() {
        return self.getAccessor.call(this, model, label);
      };
      this.addEncoders();
  ...
  Batman.Association.Collection = (function() {
    ...
    BatmanAssociationCollection = (function() {
      ...
      BatmanAssociationCollection.prototype.add = function(association) {
        var associationType, associationTypeHash, baseModelHash, baseModelName;
        baseModelName = $functionName(association.model);
        if (!(baseModelHash = this.storage.get(baseModelName))) {
          baseModelHash = new Batman.Hash;
          this.storage.set(baseModelName, baseModelHash);
        }
        associationType = $functionName(association.constructor);
        if (!(associationTypeHash = baseModelHash.get(associationType))) {
      ...
      BatmanAssociationCollection.prototype.hasAssociationOn = function(model, name) {
        var modelHash, modelName, ret;
        ret = false;
        modelName = $functionName(model.constructor);
        if (modelHash = this.storage.get(modelName)) {
          modelHash.forEach(function(type, hash) {
            if (ret === true) {
              return;
            }
      ...
      BatmanAssociationCollection.prototype.getModelAssociations = function(model) {
        var hash, modelName;
        modelName = $functionName(model.constructor);
        if (hash = this.storage.get(modelName)) {
          return {
      ...
      BatmanAssociationCollection.prototype.clearModelRelations = function(model) {
        var modelHash, modelName;
        modelName = $functionName(model.constructor);
        if (modelHash = this.storage.get(modelName)) {
          return modelHash.forEach(function(type, typeHash) {
            return typeHash.forEach(function(association, label) {
              return association.clearRelation(model);
            });
          });
        }
      };
  ...
  Batman.Association.belongsTo = (function() {
    ...
    belongsTo.prototype.getAccessor = function(model, label) {
    ...
    belongsTo.prototype.addEncoders = function() {
      this.model.encode("" + this.label + "_id");
      return this.model.encode("" + this.label, {
        decode: __bind(function(data) {
          var relatedModel, relatedModelName;
          if (typeof data === "string") {
            relatedModelName = helpers.camelize(helpers.singularize(this.label));
            relatedModel = Batman.currentApp[relatedModelName];
            return new relatedModel(JSON.parse(data));
          } else {
            return data;
          }
        }, this)
  ...
  Batman.Association.hasOne = (function() {
    ...
    hasOne.prototype.getAccessor = function(model, label) {
      var existingInstance, id, loadOptions, loadedRecord, modelName, relatedModel, relatedRecords;
      if (this.amSetting) {
        return;
      }
    ...
    hasOne.prototype.clearRelation = function(base) {
      var baseName, relatedModel;
      baseName = $functionName(base.constructor).toLowerCase();
      relatedModel = Batman.currentApp[helpers.camelize(helpers.singularize(this.label))];
      return relatedModel.get('loaded').indexedBy("" + baseName + "_id").get(base.id).forEach(function(relatedInstance) {
        relatedInstance.unset("" + baseName + "_id");
        return relatedInstance.unset(baseName);
      });
    };
    hasOne.prototype.addEncoders = function() {
      return this.model.encode(this.label, {
        decode: __bind(function(data) {
          ...
          if (typeof data === "string") {
            ...
            return relatedModel._mapIdentity(record);
          } else {
            return data;
          }
        }, this)
  ...
  Batman.Association.hasMany = (function() {
    ...
    hasMany.prototype.getAccessor = function(model, label) {
      var existingInstance, id, loadOptions, loadedRecords, modelName, recordInAttributes, relatedModel, relatedRecords, _ref;
      if (this.amSetting) {
        return;
      }
      existingInstance = Batman.Model.defaultAccessor.get.call(this, label);
    ...
    hasMany.prototype.addEncoders = function() {
      return this.model.encode(this.label, {
        decode: __bind(function(data) {
          var id, idRegex, obj, record, relatedModel, relations, storageKey, _, _ref, _ref2;
          if (typeof data === "string") {
