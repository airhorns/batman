...
(function() {
    ...
    if (typeof require === 'undefined') {
        ...
        */
        ...
        if (typeof BATMAN_LIB_PATH !== 'undefined')
            Batman.LIB_PATH = BATMAN_LIB_PATH;
        
        if (typeof Batman.LIB_PATH === 'undefined') {
    ...
    if (!Function.prototype.bind)
        Function.prototype.bind = function(context, prependArgs) {
            var slice = [].slice,
                ...
                nop = function(){},
                bound = function() {
                    return original.apply(this instanceof nop ? this : (context || {}), args.concat(slice.call(arguments)));
                };
            
            nop.prototype = original.prototype;
        ...
        for (var i = 0, count = arguments.length; ++i < count;) {
            ...
            if (arg.isMixin && arg.prototype)
                ...
                for (var key in arg) {
                    ...
                    var binding = to[key];
                    if (binding && (binding.isBinding || binding.isEvent) && !(value && value.isBinding))
    ...
    Batman.bindingless = function(object) {
        var result = {};
    ...
    Batman.Mixin.prototype = {
        ...
        enhance: function() {
            ...
            if (this.prototype.initialize) {
                this.initialize = this.prototype.initialize;
                
                this.prototype.initialize = null;
        ...
        create: function(identifier) {
            var obj = Batman(this, Array.toArray(arguments));
            
            if (typeof identifier === 'string')
                obj.identifier = identifier;
            
            if (this.initialize) {
                var result = this.initialize.apply(obj, arguments);
        ...
        copy: function(identifier) {
            var copy = Batman.Mixin(typeof identifier === 'string' && identifier, this.prototype, Array.toArray(arguments)),
                obj = this.toObject();
        ...
        toString: function() {
            var string = this.identifier || 'mixin';
    ...
    Batman.binding = function(defaultValueOrGetter, setter) {
        ...
        if (typeof defaultValueOrGetter === 'function') {
            ...
            !binding.set && (binding.set = setter || defaultValueOrGetter);
    ...
    Batman.Binding = Batman.Mixin({
        ...
        observeLater: function(func) {
            setTimeout(this.observe.bind(this, func), 0);
            return this;
        },
        ...
        dependencyFired: function(dependency) {
            this.fire(this.observeDependencies());
        },
    ...
    Batman.Request = Batman.Mixin({
        ...
        send: Batman.event(function() {
            ...
            request.onreadystatechange = function() {
                ...
                if (window.location.protocol === "file:" || (request.status >= 200 && request.status < 300))
                    this.success(request.responseText, request);
                else
                    this.error(request);
            }.bind(this);
            
            request.open(this.method().toUpperCase(), this.url(), this.async());
    ...
    Batman.functionFromString = function(string, contexts) {
        ...
        var f = new Function('contexts', withStart.join('') + 'try{return ' + string + '}catch(e){console.log(e);return\'\'}' + withEnd.join(''));
    ...
    var camelize_rx = /(?:^|_)(.)/g,
        underscore1_rx = /([A-Z]+)([A-Z][a-z])/g,
        underscore2_rx = /([a-z\d])([A-Z])/g,
        simple1_rx = /\r\n?/g, simple2_rx = /\n\n+/g,
        simple3_rx = /([^\n]\n)(?=[^\n])/g;
    
    Batman.helpers = {
        simpleFormat: textHelper(function(string, options) {
            return '<p>' + string
                .replace(simple1_rx, '\n')
                .replace(simple2_rx, '</p>\n\n<p>')
                .replace(simple3_rx, '$1<br />') + // FIXME: Removes last letter
                '</p>';
        }),
        
        autoLink: textHelper(function(string, options) {
            return string; // FIXME
        }),
        
        camelize: textHelper(function(string, firstLetterLower) {
            string = string.replace(camelize_rx, function(str, p1) { return p1.toUpperCase(); });
            return firstLetterLower ? string.substr(0,1).toLowerCase() + string.substr(1) : string;
        }),
        
        underscore: textHelper(function(string) {
            return string.replace(underscore1_rx, '$1_$2')
                         .replace(underscore2_rx, '$1_$2')
                         .replace('-', '_')
                         .toLowerCase();
        })
    }
