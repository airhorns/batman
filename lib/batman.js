(function() {
  ...
  Batman.getPath = $getPath = function(base, segments) {
    ...
    for (_i = 0, _len = segments.length; _i < _len; _i++) {
      segment = segments[_i];
      if (!((base != null) && ((base = $get(base, segment)) != null))) return;
    }
    return base;
  };
    ...
    Event.prototype.eachHandler = function(iterator) {
      ...
      if ((_ref = this.base) != null ? _ref.isEventEmitter : void 0) {
        ...
        return (_ref2 = this.base._batman) != null ? _ref2.ancestors(function(ancestor) {
          var handlers, _ref3, _ref4;
          if (ancestor.isEventEmitter && ((_ref3 = ancestor._batman) != null ? (_ref4 = _ref3.events) != null ? _ref4.hasOwnProperty(key) : void 0 : void 0)) {
  ...
  Batman.Set = (function() {
    ...
    _ref2 = ['find', 'merge', 'forEach', 'toArray', 'isEmpty', 'has'];
    _fn = function(k) {
  ...
  Batman.App = (function() {
    ...
    App.run = function() {
      ...
      if ((layout = this.get('layout')) != null) {
  ...
  Batman.RenderCache = (function() {
    ...
    RenderCache.prototype.viewForOptions = function(options) {
      ...
      return this.getOrSet(options, function() {
        return _this._newViewFromOptions(options);
      });
    };
    ...
    RenderCache.wrapAccessor(function(core) {
      return {
        ...
        get: function(key) {
          ...
          if (result) this._addOrBubbleKey(key);
          return result;
        },
    ...
    RenderCache.prototype._evictExpiredKeys = function() {
      var currentKeys, i, key, _ref, _ref2;
      if (this.length > this.maximumLength) {
        currentKeys = this.keyQueue.slice(0);
        for (i = _ref = this.maximumLength, _ref2 = currentKeys.length; _ref <= _ref2 ? i < _ref2 : i > _ref2; _ref <= _ref2 ? i++ : i--) {
          key = currentKeys[i];
          if (!this.get(key).inUse()) this.unset(key);
        }
      }
    };
  ...
  Batman.Controller = (function() {
    ...
    Controller.prototype.contstructor = function() {
      Controller.__super__.contstructor.apply(this, arguments);
      return this._renderedYields = {};
    };
    ...
    Controller.prototype.dispatch = function(action, params) {
      var name, oldRedirect, redirectTo, yield, _ref, _ref2, _ref3, _ref4;
      ...
      _ref3 = Batman.DOM.Yield.yields;
      for (name in _ref3) {
        yield = _ref3[name];
        if (!this._renderedYields[name]) yield.clear();
      }
      delete this._actedDuringAction;
      delete this._inAction;
      if ((_ref4 = Batman.navigator) != null) _ref4.redirect = oldRedirect;
    ...
    Controller.prototype.render = function(options) {
      var view, _ref, _ref2, _ref3;
      var _this = this;
      if (options == null) options = {};
      if (options) options.into || (options.into = 'main');
      if (this._actedDuringAction && this._inAction && this._renderedYields[options.into]) {
        developer.warn("Warning! Trying to render but an action has already be taken during " + (this.get('controllerName')) + "." + (this.get('action')) + " on yield " + options.into);
      }
      this._actedDuringAction = true;
      if (options === false) return;
      if ((_ref = this._renderedYields) != null) _ref[options.into] = true;
      if (!options.view) {
        options.viewClass || (options.viewClass = ((_ref2 = Batman.currentApp) != null ? _ref2[helpers.camelize("" + (this.get('controllerName')) + "_" + (this.get('action')) + "_view")] : void 0) || Batman.View);
      ...
      if (view) {
        if ((_ref3 = Batman.currentApp) != null) _ref3.prevent('ready');
  ...
  Batman.Model = (function() {
    ...
    Model.encode = function() {
      var encoder, encoderOrLastKey, hash, key, keys, operation, _base, _base2, _i, _j, _k, _len, _len2, _ref;
  ...
  Batman.View = (function() {
    ...
    View.option = function() {
      ...
      keys.forEach(function(key) {
        return _this.accessor(_this.prototype._argumentBindingKey(key), function(bindingKey) {
          ...
          if ((_ref = this[bindingKey]) != null) _ref.die();
    ...
    View.YieldStorage = (function() {

      __extends(YieldStorage, Batman.Hash);

      function YieldStorage() {
        YieldStorage.__super__.constructor.apply(this, arguments);
      }
  ...
  Batman.Renderer = (function() {
    ...
    Renderer.prototype.parseNode = function(node) {
      var argument, attribute, bindings, keypath, names, nextNode, oldContext, result, skipChildren, _base, _base2, _j, _k, _len3, _len4, _ref2, _ref3, _ref4, _ref5;
      var _this = this;
      ...
      if (node.getAttribute && node.attributes) {
        ...
        for (_j = 0, _len3 = _ref2.length; _j < _len3; _j++) {
          attribute = _ref2[_j];
          name = (_ref3 = attribute.nodeName.match(bindingRegexp)) != null ? _ref3[1] : void 0;
          if (!name) continue;
        ...
        _ref4 = bindings.sort(this._sortBindings);
        for (_k = 0, _len4 = _ref4.length; _k < _len4; _k++) {
          _ref5 = _ref4[_k], name = _ref5[0], argument = _ref5[1], keypath = _ref5[2];
  ...
  Batman.DOM = {
    readers: {
      ...
      contentfor: function(node, key, context, renderer, action) {
        if (action == null) action = 'append';
        $onParseExit(node, function() {
          var _ref;
          if ((_ref = node.parentNode) != null) _ref.removeChild(node);
    ...
    Yield: Yield = (function() {
      ...
      __extends(Yield, Batman.Object);

      function Yield() {
        Yield.__super__.constructor.apply(this, arguments);
      }
    ...
    onParseExit: $onParseExit = function(node, callback) {
      ...
      if (callback != null) set.add(callback);
      return set;
    },
    ...
    willInsertNode: function(node) {
      var child, view, _i, _len, _ref, _ref2;
      view = Batman._data(node, 'view');
      if (view != null) view.fire('beforeAppear', node);
      if ((_ref = Batman.data(node, 'show')) != null) _ref.call(node);
    ...
    willRemoveNode: function(node) {
      var child, view, _i, _len, _ref, _ref2;
      view = Batman._data(node, 'view');
      if (view) view.fire('beforeDisappear', node);
      if ((_ref = Batman.data(node, 'hide')) != null) _ref.call(node);
      _ref2 = node.childNodes;
      for (_i = 0, _len = _ref2.length; _i < _len; _i++) {
        child = _ref2[_i];
        Batman.DOM.willRemoveNode(child);
  ...
  Batman.DOM.ShowHideBinding = (function() {
    ...
    ShowHideBinding.prototype.dataChange = function(value) {
      ...
      if (!!value === !this.invert) {
        ...
        if ((_ref = Batman.data(this.node, 'show')) != null) _ref.call(this.node);
  ...
  Batman.DOM.AddClassBinding = (function() {
    ...
    AddClassBinding.prototype.dataChange = function(value) {
      var currentName, includesClassName, name, pattern, _i, _len, _ref, _ref2;
      currentName = this.node.className;
      _ref = this.classes;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        _ref2 = _ref[_i], name = _ref2.name, pattern = _ref2.pattern;
  ...
  Batman.DOM.StyleBinding = (function() {
    ...
    StyleBinding.prototype.dataChange = function(value) {
      var colonSplitCSSValues, cssName, key, keyValue, keypathValue, style, _i, _len, _ref, _ref2, _results;
      var _this = this;
      if (!value) {
  ...
  Batman.DOM.IteratorBinding = (function() {
    ...
    IteratorBinding.prototype.removeItem = function(item) {
      ...
      if (oldNode) return $destroyNode(oldNode);
    };
      ...
      if (existingActionNumber > options.actionNumber) {
        ...
        this.actions[options.actionNumber] = function() {
          ...
          if (addItem = node.getAttribute('data-additem')) {
            return (_ref = this.renderer.context.contextForKey(addItem)) != null ? typeof _ref[addItem] === "function" ? _ref[addItem](item, node) : void 0 : void 0;
          }
        };
  ...
  Batman.DOM.ViewArgumentBinding = (function() {

    __extends(ViewArgumentBinding, Batman.DOM.AbstractBinding);

    function ViewArgumentBinding() {
      ViewArgumentBinding.__super__.constructor.apply(this, arguments);
    }
