(function() {
    ...
    Model.prototype.save = function(callback) {
      ...
      return this.validate(__bind(function(isValid, errors) {
        ...
        if (associations = this.constructor.associations) {
          ...
          if (belongsTo != null) {
            belongsTo.forEach(__bind(function(key) {
              var model;
              if ((model = this.get(key)) && model.state() === "dirty") {
                return model.save(__bind(function(err, record) {
                  if (err) {
                    throw err;
                  }
                  this.set("" + key + "_id", record.id);
                  return modelSaved.allow();
                }, this));
              } else {
                if (model) {
                  this.set("" + key + "_id", model.id);
                }
                return modelSaved.allow();
              }
            }, this));
        ...
        return this._doStorageOperation((creating ? 'create' : 'update'), {}, __bind(function(mainErr, record) {
          if (!mainErr) {
            if (creating) {
              this.created();
            }
            ...
            record = this.constructor._mapIdentity(record);
            if (hasOne != null) {
              hasOne.forEach(__bind(function(key) {
                var foreignKey, model, _ref5;
                if (model = (_ref5 = record._batman.attributes) != null ? _ref5[key] : void 0) {
                  foreignKey = $functionName(this.constructor).toLowerCase() + "_id";
                  model.set(foreignKey, this.id);
                  if (model.state() === "dirty") {
                    return model.save(__bind(function(err, hasOneRecord) {
                      if (err) {
                        throw err;
                      }
                      return modelSaved.allowAndFire(mainErr, record);
                    }, this));
                  } else {
                    return modelSaved.allowAndFire(mainErr, record);
                  }
            ...
            if (hasMany != null) {
              hasMany.forEach(__bind(function(key) {
                var hasManySet, _ref5;
                if (hasManySet = (_ref5 = record._batman.attributes) != null ? _ref5[key] : void 0) {
                  return hasManySet.forEach(__bind(function(model) {
                    var foreignKey;
                    foreignKey = $functionName(this.constructor).toLowerCase() + "_id";
                    model.set(foreignKey, this.id);
                    if (model.state() === "dirty") {
                      return model.save(__bind(function(err, hasManyRecord) {
                        if (err) {
                          throw err;
                        }
                        return modelSaved.allowAndFire(mainErr, record);
  ...
  Batman.Association.belongsTo = (function() {
    function belongsTo(model, label, relatedModel) {
      ...
      (_base = model.associations).belongsTo || (_base.belongsTo = new Batman.Hash);
  ...
  Batman.Association.hasOne = (function() {
    function hasOne(model, label, relatedModel) {
      var _base;
      model.associations || (model.associations = {});
      (_base = model.associations).hasOne || (_base.hasOne = new Batman.Hash);
  ...
  Batman.Association.hasMany = (function() {
    function hasMany(model, label, relatedModel) {
      var _base;
      model.associations || (model.associations = {});
      (_base = model.associations).hasMany || (_base.hasMany = new Batman.Hash);
