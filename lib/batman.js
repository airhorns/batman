(function() {
  ...
  Batman.Property = (function() {
    ...
    Property.accessorForBaseAndKey = function(base, key) {
      var keyAccessors, val, _ref;
      keyAccessors = (_ref = base._batman) != null ? _ref.get('keyAccessors') : void 0;
      if (keyAccessors && (val = keyAccessors.get(key))) {
        return val;
      } else {
        return this.defaultAccessorForBase(base);
      }
    };
  ...
  Batman.PromiseAccessor = (function() {

    function PromiseAccessor(fetcher, wrappedAccessor) {
      var k, v;
      if (wrappedAccessor == null) {
        wrappedAccessor = Batman.Property.defaultAccessor;
      }
      for (k in wrappedAccessor) {
        v = wrappedAccessor[k];
        this[k] = v;
      }
  ...
  Batman.AssociationSet = (function() {
    ...
    function AssociationSet(value, association) {
      var base;
      this.value = value;
      this.association = association;
      base = new Batman.Set;
    ...
    AssociationSet.prototype.load = function(callback) {
      var loadOptions;
      var _this = this;
      if (this.value == null) return callback(void 0, this);
      loadOptions = {};
      loadOptions[this.association.foreignKey] = this.value;
  ...
  Batman.DOM = {
    ...
    contentFor: function(name, node, _replaceContent, _yieldChildren) {
      ...
      if (yieldingNode && $isChildOf(yieldingNode, node)) node = $cloneNode(node);
    ...
    copyNodeEventListeners: $copyNodeEventListeners = function(dst, src) {
      var eventListeners, eventName, listeners, _results;
      if (listeners = Batman._data(src, 'listeners')) {
        _results = [];
        for (eventName in listeners) {
          eventListeners = listeners[eventName];
          _results.push(eventListeners.forEach(function(listener) {
            return $addEventListener(dst, eventName, listener);
          }));
        }
        return _results;
      }
    },
    copyTreeEventListeners: $copyTreeEventListeners = function(dst, src) {
      var i, _ref2, _results;
      $copyNodeEventListeners(dst, src);
      _results = [];
      for (i = 0, _ref2 = src.childNodes.length; 0 <= _ref2 ? i < _ref2 : i > _ref2; 0 <= _ref2 ? i++ : i--) {
        _results.push($copyTreeEventListeners(dst.childNodes[i], src.childNodes[i]));
      }
      return _results;
    },
    cloneNode: $cloneNode = function(node, deep) {
      var newNode;
