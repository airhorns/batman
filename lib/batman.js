(function() {
  ...
  Batman.Controller = (function() {
    ...
    Controller.prototype.render = function(options) {
      ...
      if (!options.view) {
        ...
        options.source || (options.source = helpers.underscore($functionName(this.constructor).replace('Controller', '')) + '/' + this._currentAction);
      ...
      if (view = options.view) {
        ...
        view.on('ready', function() {
          ...
          return (_ref2 = Batman.currentApp) != null ? _ref2.allowAndFire('ready') : void 0;
        });
      }
      return view;
    };
  ...
  Batman.Model = (function() {
    ...
    Model.hasOne = function(label, model) {
      return new Batman.Association.hasOne(this, label, model);
    };
    Model.belongsTo = function(label, model) {
      return new Batman.Association.belongsTo(this, label, model);
    };
    Model.hasMany = function(label, model) {
      return new Batman.Association.hasMany(this, label, model);
    };
    Model.accessor('id', {
      get: function() {
        var pk;
    ...
    Model.prototype.toJSON = function() {
      ...
      if (!(!encoders || encoders.isEmpty())) {
        encoders.forEach(__bind(function(key, encoder) {
          var encodedVal, val, _ref;
          if ((_ref = this.constructor.associations) != null ? _ref.has(key) : void 0) {
            return;
          }
          val = this.get(key);
    ...
    Model.prototype.save = function(callback) {
      ...
      return this.validate(__bind(function(isValid, errors) {
        var associations, belongsTo, creating, hasMany, hasOne, modelSaved, _ref4;
        if (!isValid) {
          if (typeof callback === "function") {
            callback(errors);
          }
        ...
        modelSaved = this.event('modelSaved');
        modelSaved.prevent();
        modelSaved.addHandler(function(err, savedRecord) {
          return typeof callback === "function" ? callback(err, savedRecord) : void 0;
        });
        if (associations = this.constructor.associations) {
          _ref4 = associations.getObject(), belongsTo = _ref4.belongsTo, hasOne = _ref4.hasOne, hasMany = _ref4.hasMany;
          if (belongsTo != null) {
            belongsTo.forEach(__bind(function(association) {
              return association.save(this);
            }, this));
          }
        }
        return this._doStorageOperation((creating ? 'create' : 'update'), {}, __bind(function(storageOpError, record) {
          var k, _k, _len3, _ref5;
          if (!storageOpError) {
            if (creating) {
              this.created();
            }
            ...
            record = this.constructor._mapIdentity(record);
            _ref5 = [hasOne, hasMany];
            for (_k = 0, _len3 = _ref5.length; _k < _len3; _k++) {
              k = _ref5[_k];
              if (k != null) {
                k.forEach(function(association) {
                  return association.save(storageOpError, record);
                });
              }
            }
          }
          return modelSaved.allowAndFire(storageOpError, record);
        }, this));
      }, this));
    };
    Model.prototype.destroy = function(callback) {
      ...
      return this._doStorageOperation('destroy', {}, __bind(function(err, record) {
        var _ref3;
        if ((_ref3 = record.constructor.associations) != null) {
          _ref3.clearRelations(this);
        }
        if (!err) {
  ...
  Batman.Association = (function() {
    function Association(model, label, relatedModel) {
      var getAccessor, self;
      this.model = model;
      this.label = label;
      this.relatedModel = relatedModel;
      model.associations || (model.associations = new Batman.Association.Collection);
      model.associations.add(this);
      this.addEncoder();
      self = this;
      getAccessor = function() {
        return self.getAccessor.call(this, model, label, relatedModel);
      };
    ...
    Association.prototype.addEncoder = function() {
      return developer.error("You must override addEncoder in Batman.Association subclasses.");
    };
    Association.prototype.clearRelation = function() {
      return developer.error("You must override clearRelation in Batman.Association subclasses.");
    };
    return Association;
  })();
  Batman.Association.Collection = (function() {
    function Collection() {
      this.storage = new Batman.Hash;
    }
    Collection.prototype.add = function(association) {
      var hash, type;
      type = $functionName(association.constructor);
      if (!(hash = this.storage.get(type))) {
        hash = new Batman.Hash;
        this.storage.set(type, hash);
      }
      return hash.set(association, association.label);
    };
    Collection.prototype.has = function(name) {
      var ret;
      ret = false;
      this.storage.forEach(function(type, hash) {
        return hash.forEach(function(association, label) {
          if (label === name) {
            return ret = true;
          }
        });
    ...
    Collection.prototype.clearRelations = function(base) {
      return this.storage.forEach(function(type, hash) {
        return hash.forEach(function(association, label) {
          return association.clearRelation(base);
        });
      });
    };
    Collection.prototype.getObject = function() {
      return {
        belongsTo: this.storage.get('belongsTo'),
        hasOne: this.storage.get('hasOne'),
        hasMany: this.storage.get('hasMany')
      };
    };
    return Collection;
  })();
  Batman.Association.belongsTo = (function() {
    ...
    belongsTo.prototype.getAccessor = function(model, label, relatedModel) {
      var loadedRecord, relatedID, relatedRecord, _ref;
      if (relatedRecord = (_ref = this._batman.attributes) != null ? _ref[label] : void 0) {
        return relatedRecord;
      } else if (relatedID = this.get("" + label + "_id")) {
    ...
    belongsTo.prototype.save = function(base) {
      var model, saveEvent;
      saveEvent = base.event('modelSaved');
      saveEvent.prevent();
      model = base.get(this.label);
      if ((model != null ? model.state() : void 0) === "dirty") {
        return model.save(__bind(function(err, record) {
          if (err) {
            throw err;
          }
          base.set("" + this.label + "_id", record.id);
          return saveEvent.allow();
        }, this));
      } else {
        if (model) {
          base.set("" + this.label + "_id", model.id);
        }
        return saveEvent.allow();
      }
    };
    belongsTo.prototype.clearRelation = function(base) {};
    belongsTo.prototype.addEncoder = function() {
      this.model.encode("" + this.label + "_id");
      return this.model.encode("" + this.label, {
        decode: __bind(function(data) {
          if (typeof data === "string") {
            return new this.relatedModel(JSON.parse(data));
          } else {
            return data;
          }
        }, this)
  ...
  Batman.Association.hasOne = (function() {
    ...
    hasOne.prototype.getAccessor = function(model, label, relatedModel) {
      var existingInstance, id, loadOptions, loadedRecord, modelName, relatedRecords;
      if (this.amSetting) {
        return;
      }
      ...
      if (!relatedRecords.isEmpty()) {
        ...
        relatedModel.load(loadOptions, __bind(function(error, loadedRecords) {
          ...
          return this.amSetting = false;
        }, this));
        return loadedRecord;
      }
    };
    hasOne.prototype.save = function(baseSaveError, base) {
      var foreignKey, relatedModel, saveEvent, _ref;
      saveEvent = base.event('modelSaved');
      saveEvent.prevent();
      if (relatedModel = (_ref = base._batman.attributes) != null ? _ref[this.label] : void 0) {
        foreignKey = $functionName(base.constructor).toLowerCase() + "_id";
        relatedModel.set(foreignKey, base.id);
        if (relatedModel.state() === "dirty") {
          return relatedModel.save(__bind(function(err, relatedRecord) {
            if (err) {
              throw err;
            }
            return saveEvent.allowAndFire(baseSaveError, base);
          }, this));
        } else {
          return saveEvent.allowAndFire(baseSaveError, base);
        }
    ...
    hasOne.prototype.clearRelation = function(base) {
      var baseName;
      baseName = $functionName(base.constructor).toLowerCase();
      return this.relatedModel.get('loaded').indexedBy(baseName + "_id").get(base.id).forEach(function(relatedInstance) {
        relatedInstance.unset(baseName + "_id");
        return relatedInstance.unset(baseName);
      });
    };
    hasOne.prototype.addEncoder = function() {
      this.relatedModel.encode("" + this.label + "_id");
      return this.model.encode(this.label, {
        decode: __bind(function(data) {
          if (typeof data === "string") {
            return new this.relatedModel(JSON.parse(data));
  ...
  Batman.Association.hasMany = (function() {
    ...
    hasMany.prototype.getAccessor = function(model, label, relatedModel) {
      var existingInstance, id, loadOptions, loadedRecords, modelName, recordInAttributes, relatedRecords, _ref;
      if (this.amSetting) {
        return;
      }
      existingInstance = Batman.Model.defaultAccessor.get.call(this, label);
      ...
      if (!relatedRecords.isEmpty()) {
      ...
      } else if (recordInAttributes = (_ref = this._batman.attributes) != null ? _ref[label] : void 0) {
        ...
        this.amSetting = true;
        this.set(label, loadedRecords);
        loadOptions = {};
        loadOptions[modelName + '_id'] = id;
        relatedModel.load(loadOptions, __bind(function(error, records) {
          var record, _i, _len;
          if (error) {
            throw error;
          }
    ...
    hasMany.prototype.save = function(baseSaveError, base) {
      var relatedModels, saveEvent, _ref;
      saveEvent = base.event('modelSaved');
      if (relatedModels = (_ref = base._batman.attributes) != null ? _ref[this.label] : void 0) {
        return relatedModels.forEach(__bind(function(model) {
          var foreignKey;
          saveEvent.prevent();
          foreignKey = $functionName(base.constructor).toLowerCase() + "_id";
          model.set(foreignKey, base.id);
          if (model.state() === "dirty") {
            return model.save(__bind(function(err, relatedRecord) {
              if (err) {
                throw err;
              }
              return saveEvent.allowAndFire(baseSaveError, base);
    ...
    hasMany.prototype.clearRelation = Batman.Association.hasOne.prototype.clearRelation;
    hasMany.prototype.addEncoder = function() {
      this.relatedModel.encode("" + this.label + "_id");
      return this.model.encode(this.label, {
        decode: __bind(function(data) {
          var id, idRegex, jsonCollection, model, obj, relatedModelName, relations, storageKey, _, _ref;
          if (typeof data === "string") {
            this.model._readingFromJSON = true;
            jsonCollection = JSON.parse(data);
            relations = new Batman.Set;
            relatedModelName = $functionName(this.relatedModel).toLowerCase() + "s";
            idRegex = new RegExp("^" + relatedModelName + "(\\d+)$");
            for (storageKey in jsonCollection) {
              if (!__hasProp.call(jsonCollection, storageKey)) continue;
              obj = jsonCollection[storageKey];
              _ref = storageKey.match(idRegex), _ = _ref[0], id = _ref[1];
              model = new this.relatedModel(obj);
              model.set('id', id);
              relations.add(model);
            }
            return relations;
