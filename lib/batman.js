    ...
    View.prototype.observe('node', function(node) {
      ...
      if (node) {
        return this._renderer = new Batman.Renderer(node, __bind(function() {
          if (this.contentFor) {
            Batman.DOM.contentFor(this.contentFor, node);
          }
  ...
  Batman.DOM = {
    readers: {
      ...
      route: function(node, key, contexts) {
        var context, controller, controllerName, index, route, routeName;
        if (key.substr(0, 1) === '/') {
    ...
    attrReaders: {
      bind: function(node, attr, key, contexts) {
        var context, filter, filterName, filters, _results;
      ...
      foreach: function(node, iteratorName, key, contexts) {
        var context;
        context = matchContext(contexts, key);
        return context.observe(key, true, function(collection) {});
      }
    },
    events: {
