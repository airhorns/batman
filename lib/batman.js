(function() {
  ...
  Batman.ModelDraft = (function() {
    ...
    ModelDraft.prototype.destroy = function(callback) {
      return this.parent.destroy(__bind(function(err) {
        if (!err) {
    ...
    ModelDraft.prototype._willSet = function(key) {
      ...
      if (this.get('lifecycle')["do"]('set')) {
        return this.set("dirtyKeys." + key, true);
      } else {
        throw new Batman.StateMachine.InvalidTransitionError("Can't set while in state " + (this.get('lifecycle.state')));
      }
    };
    ModelDraft.prototype._doStorageOperation = function(operation, options, callback) {
      ...
      mechanisms = this.record()._batman.get('storage');
      ...
      if (this.get('lifecycle').destroy()) {
        return this._doStorageOperation('destroy', {}, __bind(function(err, record) {
          if (!err) {
            ...
            this.get('lifecycle').destroyed();
            this.get('drafts').forEach(function(draft) {
              return draft.get('lifecycle').destroyed();
            });
          } else {
            this.get('lifecycle').error();
          }
