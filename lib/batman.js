(function() {
  ...
  Batman.Object = (function() {
    ...
    Object.singleton = function(singletonMethodName) {
      var obj;
      obj = {};
      obj[singletonMethodName] = function() {
        var _name;
        return this[_name = "_" + singletonMethodName] || (this[_name] = new this);
      };
      return this.classAccessor(obj);
    };
    return Object;
  })();
  ...
  Batman.App = (function() {
    ...
    App.run = App.eventOneShot(function() {
      ...
      this.dispatcher = new Batman.Dispatcher(this);
      this.historyManager || (this.historyManager = new Batman.HashHistory(this));
  ...
  Batman.Route = (function() {
    ...
    Route.accessor('action', {
      get: function() {
        var action, components, controller;
        if (this.action) {
          return this.action;
        }
        if (this.signature) {
          components = this.signature.split('#');
          if (components.length === 1) {
            components[1] = 'index';
          }
          controller = this.dispatcher.get(components[0]);
          action = components[1];
          return this.set('action', [controller, action]);
        }
      },
    ...
    Route.prototype.dispatch = function(params) {
      var action, controller, _ref2;
      action = this.get('action');
      if (typeof action === 'function') {
        return action(params);
      }
      _ref2 = action, controller = _ref2[0], action = _ref2[1];
  ...
  Batman.Dispatcher = (function() {
    ...
    function Dispatcher(app) {
      ...
      this.app = app;
      this.app._flushToDispatcher(this);
      _ref2 = this.app;
      for (key in _ref2) {
    ...
    Dispatcher.prototype.prepareController = function(controller) {
      ...
      if (name) {
        return this.accessor(name, {
          get: function() {
            return this[name] = controller.sharedController();
          }
        });
      }
    };
    Dispatcher.prototype.register = function(url, route) {
      var typeOf;
      if (url.indexOf('/') !== 0) {
        url = "/" + url;
      }
      if ((typeOf = $typeOf(route)) === 'String') {
        route = new Batman.Route({
          url: url,
          signature: route,
          dispatcher: this
        });
    ...
    Dispatcher.prototype.parameterizeRoute = function(url, route) {
      var array, index, key, param, params, query, s, value, _j, _len2, _len3, _ref2, _ref3, _ref4, _ref5;
      _ref2 = url.split('?'), url = _ref2[0], query = _ref2[1];
      array = (_ref3 = route.regexp.exec(url)) != null ? _ref3.slice(1) : void 0;
      params = {
        url: url
      };
      if (array) {
    ...
    Dispatcher.prototype.dispatch = function(url) {
      ...
      return route.dispatch(this.parameterizeRoute(url, route));
    };
    return Dispatcher;
  })();
  Batman.HistoryManager = (function() {
    function HistoryManager(app) {
      this.app = app;
      Batman.historyManager = this;
      if (this.start && this.app.dispatcher) {
    ...
    HistoryManager.prototype.dispatch = function(url) {
      ...
      return this.app.dispatcher.dispatch(url);
    };
  ...
  Batman.HashHistory = (function() {
    ...
    HashHistory.prototype.redirect = function(url) {
      HashHistory.__super__.redirect.apply(this, arguments);
      if (url.indexOf('/') !== 0) {
        url = "/" + url;
      }
      this.cachedHash = url;
  ...
  Batman.redirect = $redirect = function(url) {
    return Batman.historyManager.redirect(url);
  };
  Batman.App.classMixin({
    _flushToDispatcher: function(url, route) {
      var dispatcher, key, value, _ref2;
      if ($typeOf(url) === 'String') {
        this._dispatcherCache || (this._dispatcherCache = {});
        return this._dispatcherCache[url] = route;
      } else if (url) {
        dispatcher = url;
        ...
        return this._dispatcherCache = null;
      }
    },
    route: function(url, signature, options) {
      return this._flushToDispatcher(url, signature);
    },
    root: function(signature) {
      return this.route('/', signature);
    },
    resources: function(resource, options, callback) {
      var app, controller, obj;
