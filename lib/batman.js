(function() {
  ...
  Batman.mixin = $mixin = function() {
    var hasSet, iterator, key, mixin, mixins, set, to, value, _i, _len;
    ...
    for (_i = 0, _len = mixins.length; _i < _len; _i++) {
      ...
      set = hasSet ? function(key, value) {
        return to.set(key, value);
      } : to.nodeName != null ? function(key, value) {
        return Batman.data(to, key, value);
      } : function(key, value) {
        return to[key] = value;
      };
      ...
      if (Batman.SimpleHash && (mixin instanceof Batman.SimpleHash || mixin instanceof Batman.Hash)) {
        mixin.forEach(iterator);
      } else {
        for (key in mixin) {
  ...
  Batman.StateMachine = (function() {
    ...
    StateMachine.prototype["do"] = function(event) {
  ...
  Batman.ModelDraft = (function() {
    __extends(ModelDraft, Batman.Object);
    ModelDraft.LifecycleStateMachine = (function() {
    ...
    function ModelDraft(parent) {
      this.parent = parent;
      ModelDraft.__super__.constructor.call(this);
    }
    ModelDraft.accessor('lifecycle', function() {
      return this.lifecycle || (this.lifecycle = new Batman.ModelDraft.LifecycleStateMachine(this.parent.get('lifecycle.state')));
    });
    ModelDraft.accessor('attributes', function() {
      return this.attributes || (this.attributes = new Batman.Hash);
    });
    ModelDraft.accessor('dirtyKeys', function() {
      return this.dirtyKeys || (this.dirtyKeys = new Batman.Hash);
    });
    ModelDraft.accessor('errors', function() {
      return this.errors || (this.errors = new Batman.ErrorsSet);
    });
    ModelDraft.accessor(ModelDraft.defaultAccessor = {
      get: function(k) {
        var local;
        if ((local = this.get("attributes." + k)) != null) {
          return local;
        } else {
      ...
      set: function(k, v) {
        if (this._willSet(k)) {
          return this.set("attributes." + k, v);
        } else {
          return v;
        }
      },
      unset: function(k) {
        if (this.get("attributes." + k) != null) {
          return this.unset("attributes." + k);
        } else {
          return this.parent.unset(k);
        }
      }
    });
    ModelDraft.prototype.isDraft = true;
    ModelDraft.prototype.draft = function() {
      var draft;
      draft = new Batman.ModelDraft(this);
      this.record().get('drafts').add(draft);
      return draft;
    };
    ModelDraft.prototype.discard = function() {
      return this.record().get('drafts').remove(this);
    };
    ModelDraft.prototype.updateAttributes = function(attrs) {
      this.mixin(attrs);
      return this;
    };
    ModelDraft.prototype.isNew = function() {
      return typeof this.get('id') === 'undefined';
    };
    ModelDraft.prototype.hasStorage = function() {
      return (this.record()._batman.get('storage') || []).length > 0;
    };
    ModelDraft.prototype.toString = function() {
      return "" + ($functionName(this.record().constructor)) + ": " + (this.get('id'));
    };
    ModelDraft.prototype.record = function() {
      var record;
      record = this.parent;
    ...
    ModelDraft.prototype.toJSON = function() {
      var encoders, obj;
      obj = {};
      Batman.Association.Collection.encodeModelIntoObject(this, obj);
      encoders = this.record()._batman.get('encoders');
    ...
    ModelDraft.prototype.validate = function(callback) {
      var args, count, errors, finishedValidation, key, validator, validators, _i, _j, _len, _len2, _ref, _results;
      validators = this.record()._batman.get('validators');
    ...
    ModelDraft.prototype.save = function(callback) {
      var action, isNew, parent, parents;
      isNew = this.isNew();
      parent = this;
      parents = ((function() {
        var _results;
      ...
      return this.validate(__bind(function(error, errors) {
        var associations, belongsTo, draft, hasMany, hasOne, _i, _len;
        ...
        if (this.get('lifecycle')["do"](action)) {
          for (_i = 0, _len = parents.length; _i < _len; _i++) {
            draft = parents[_i];
            draft.get('lifecycle')["do"](action);
          }
          ...
          return this._doStorageOperation((isNew ? 'create' : 'update'), {}, __bind(function(err, draft) {
            var record, _j, _k, _l, _len2, _len3, _len4;
            if (err) {
              for (_j = 0, _len2 = parents.length; _j < _len2; _j++) {
                draft = parents[_j];
                draft.get('lifecycle').error();
              }
            } else {
              this.get('dirtyKeys').clear();
              if (hasOne != null) {
                hasOne.forEach(function(association) {
                  return association.apply(err, draft);
                });
              }
              if (hasMany != null) {
              ...
              for (_k = 0, _len3 = parents.length; _k < _len3; _k++) {
                draft = parents[_k];
                draft.discard();
              }
              record = this.record().constructor._mapIdentity(this);
              if (!~parents.indexOf(record)) {
                parents.push(record);
              }
              for (_l = 0, _len4 = parents.length; _l < _len4; _l++) {
                draft = parents[_l];
                draft._pauseDirtyTracking = true;
                draft.updateAttributes(this.get('attributes'));
                draft._pauseDirtyTracking = false;
                draft.get('lifecycle')["do"](isNew ? 'created' : 'saved');
              }
            }
    ...
    ModelDraft.prototype.load = function(callback) {
      var _ref;
      if ((_ref = this.get('lifecycle.state')) === 'destroying' || _ref === 'destroyed') {
        if (typeof callback === "function") {
      ...
      if (this.get('lifecycle').load()) {
        this.record()._doStorageOperation('read', {}, __bind(function(err, record) {
          if (!err) {
            this.get('lifecycle').loaded();
            record = this.record().constructor._mapIdentity(this);
          } else {
            this.get('lifecycle').error();
          }
          return typeof callback === "function" ? callback(err, record) : void 0;
        }, this));
      } else {
        if (typeof callback === "function") {
          callback(new Batman.StateMachine.InvalidTransitionError("Can't load while in state " + (this.get('lifecycle.state'))));
        }
      }
    };
    ModelDraft.prototype.destroy = function(callback) {
      return this.parent.destroy(callback);
    };
    ModelDraft.prototype._willSet = function(key) {
      if (this._pauseDirtyTracking) {
        return true;
      }
      if (this.get('lifecycle')["do"]('set')) {
        this.set("dirtyKeys." + key, true);
        return true;
      } else {
        return false;
      }
    };
    ModelDraft.prototype._doStorageOperation = function(operation, options, callback) {
      var mechanism;
      ...
      mechanism = this.record()._batman.get('storage')[0];
      mechanism[operation](this, options, __bind(function() {
        callback.apply(null, arguments);
        return this._pauseDirtyTracking = false;
      }, this));
      return true;
    };
    return ModelDraft;
  ...
  Batman.Model = (function() {
    __extends(Model, Batman.ModelDraft);
    Model.primaryKey = 'id';
    ...
    Model.find = function(id, callback) {
      ...
      record = new this();
      record._pauseDirtyTracking = true;
      record.set('id', id);
      newRecord = this._mapIdentity(record);
      record._pauseDirtyTracking = false;
    ...
    Model._mapIdentity = function(record) {
      ...
      if (typeof (id = record.get('id')) === 'undefined' || id === '') {
        ...
        if (existing) {
          existing._pauseDirtyTracking = true;
          existing.updateAttributes(record.get('attributes') || {});
          existing._pauseDirtyTracking = false;
          return existing;
        } else {
          this.get('loaded').add(record.record());
          return record;
        }
      }
    };
    ...
    Model.accessor('lifecycle', function() {
      return this.lifecycle || (this.lifecycle = new Batman.ModelDraft.LifecycleStateMachine('clean'));
    });
    Model.accessor('drafts', function() {
      return this.drafts || (this.drafts = new Batman.Set);
    });
    Model.accessor(Model.defaultAccessor = {
      get: function(k) {
        var attribute;
        attribute = this.get("attributes." + k);
        if (attribute != null) {
          return attribute;
        } else {
          return this[k];
    ...
    function Model(idOrAttributes) {
      ...
      if ($typeOf(idOrAttributes) === 'Object') {
        Batman.Object.call(this, idOrAttributes);
      } else {
        Batman.Object.call(this);
    ...
    Model.prototype.destroy = function(callback) {
      if (this.get('lifecycle').destroy()) {
        this.get('drafts').forEach(function(draft) {
          return draft.get('lifecycle').destroy();
        });
        return this._doStorageOperation('destroy', {}, __bind(function(err, record) {
          if (!err) {
            this.constructor.get('loaded').remove(this);
            this.get('lifecycle').destroyed();
            this.get('drafts').forEach(function(draft) {
              draft.get('lifecycle').destroyed();
              return draft.discard();
            });
          } else {
            this.get('lifecycle').error();
            this.get('drafts').forEach(function(draft) {
              return draft.get('lifecycle').error();
            });
          }
          return typeof callback === "function" ? callback(err) : void 0;
        }, this));
  ...
  Batman.Association.belongsTo = (function() {
    ...
    belongsTo.prototype.getAccessor = function(self, model, label) {
      ...
      if (relatedRecord = this.constructor.defaultAccessor.get.call(this, label)) {
  ...
  Batman.Association.hasOne = (function() {
    ...
    hasOne.prototype.getAccessor = function(self, model, label) {
      ...
      existingInstance = this.constructor.defaultAccessor.get.call(this, label);
  ...
  Batman.Association.hasMany = (function() {
    ...
    hasMany.prototype.getAccessor = function(self, model, label) {
      var id, loadOptions, recordInAttributes, relatedModel, relatedRecords;
      if (this.amSetting) {
        return;
      }
      if (!(relatedModel = self.getRelatedModel())) {
      ...
      if (recordInAttributes = this.constructor.defaultAccessor.get.call(this, label)) {
