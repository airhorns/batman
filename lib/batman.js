    ...
    attrReaders: {
      ...
      foreach: function(node, iteratorName, key, context, parentRenderer) {
        ...
        context.bind(node, key, function(collection) {
          ...
          observers.reorder = function(items) {
            var existingNode, item, show, trackingNodeMap, _j, _len2;
            ...
            return nodeMap.forEach(function(item, node) {
              if (!trackingNodeMap.hasKey(item)) {
                return observers.remove(item);
              }
            });
          };
