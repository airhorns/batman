(function() {
  ...
  Batman.StateMachine = (function() {
    ...
    StateMachine.prototype["do"] = function(event) {
      ...
      if (this.nextEvents.length > 0) this["do"](this.nextEvents.shift());
      return true;
    };

    StateMachine.prototype.canDo = function(event, fromState) {
      if (fromState == null) fromState = this.get('state');
  ...
  Batman.Model = (function() {
    ...
    Model.accessor('lifecycle', function() {
      return this.lifecycle || (this.lifecycle = new Batman.Model.InstanceLifecycleStateMachine('clean'));
    });
    ...
    Model.prototype.load = function(options, callback) {
      var callbackQueue, hasOptions, _ref, _ref2;
      var _this = this;
      ...
      if (this.get('lifecycle').load()) {
        callbackQueue = [];
        if (callback != null) callbackQueue.push(callback);
        if (!hasOptions) this._currentLoad = callbackQueue;
        return this._doStorageOperation('read', options, function(err, record) {
          ...
          if (!hasOptions) _this._currentLoad = null;
          for (_i = 0, _len = callbackQueue.length; _i < _len; _i++) {
            callback = callbackQueue[_i];
            callback(err, record);
          }
        });
      } else {
        if (this.get('lifecycle.state') === 'loading' && !hasOptions) {
          if (callback != null) return this._currentLoad.push(callback);
        } else {
    ...
    Model.prototype.save = function(options, callback) {
      var endState, isNew, startState, storageOperation, _ref, _ref2, _ref3;
      var _this = this;
      if (!callback) _ref = [{}, options], options = _ref[0], callback = _ref[1];
      ...
      return this.validate(function(error, errors) {
        ...
        if (error || errors.length) {
          if (typeof callback === "function") callback(error || errors);
          return;
        }
        creating = _this.isNew();
        if (_this.get('lifecycle')["do"](startState)) {
          associations = _this.constructor._batman.get('associations');
          _this._pauseDirtyTracking = true;
          ...
          return _this._doStorageOperation(storageOperation, options, function(err, record) {
            var _ref5, _ref6;
            if (!err) {
              ...
              if (associations) {
                ...
                if ((_ref6 = associations.getByType('hasMany')) != null) {
                  _ref6.forEach(function(association, label) {
                    return association.apply(err, record);
                  });
                }
              }
              record = _this.constructor._mapIdentity(record);
              _this.get('lifecycle')["do"](endState);
            } else {
              _this.get('lifecycle').error();
            }
    ...
    Model.prototype.validate = function(callback) {
      var args, count, errors, finishedValidation, key, validator, validators, _i, _j, _len, _len2, _ref;
      errors = this.get('errors');
      errors.clear();
      validators = this._batman.get('validators') || [];
      if (!validators || validators.length === 0) {
        if (typeof callback === "function") callback(void 0, errors);
        return true;
      }
      ...
      for (_i = 0, _len = validators.length; _i < _len; _i++) {
        ...
        for (_j = 0, _len2 = _ref.length; _j < _len2; _j++) {
          ...
          try {
            ...
            if (typeof callback === "function") callback(e, errors);
          }
        }
      }
    };
    ...
    Model.prototype._willSet = function(key) {
      var _this = this;
      if (this._pauseDirtyTracking) return true;
