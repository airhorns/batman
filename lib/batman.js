(function() {
    ...
    Batman.mixin = function(to) {
        ...
        for (var i = 0, count = arguments.length; ++i < count;) {
            ...
            if (use)
                for (var key in use)
                    if (to[key]) {
                        Batman.Mixin.use(to, use[key], key);
                        delete use[key];
                    }
        }
    ...
    Batman.Mixin.prototype = {
        ...
        applyTo: function(to) {
            ...
            if (this.use) {
                ...
                for (var key in this.use)
                    to._use[key] = this.use[key];
            }
    ...
    Batman.Mixin.use = function(object, action, key) {
        if (action.require)
            require(action.require, function() {
                if (action.callback)
                    Batman.Mixin.use(object, action.callback, key);
            });
