(function() {
  var $block, $event, $eventOneShot, $findName, $mixin, $redirect, $route, $typeOf, $unmixin, Batman, Binding, RenderContext, Validators, camelize_rx, capitalize_rx, container, escapeRegExp, filters, helpers, mixins, namedOrSplat, namedParam, queryParam, splatParam, underscore_rx1, underscore_rx2, _Batman, _class, _class2, _objectToString, _stateMachine_setState;
  ...
  if (!Function.prototype.bind) {
    Function.prototype.bind = function() {
      ...
      return f = __bind(function() {
        return this.apply(context, args.concat(__slice.call(arguments)));
      }, this);
    };
  }
  Batman = function() {
  ...
  Batman.Property = (function() {
    ...
    Property.prototype.accessor = function() {
      var accessors, key, val, _ref, _ref2;
      key = this.key;
  ...
  */
  ...
  _stateMachine_setState = function(newState) {
    ...
    if (newState && oldState) {
      ...
      _ref = this._batman.getAll(function(ancestor) {
        var _ref, _ref2;
        return (_ref = ancestor._batman) != null ? (_ref2 = _ref.get('states')) != null ? _ref2.get(name) : void 0 : void 0;
      });
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        event = _ref[_i];
        if (event) {
  ...
  Batman._Batman = _Batman = (function() {
    ...
    _Batman.prototype.ancestors = function(getter) {
      var cons, isClass, parent, results, val, _ref;
      if (getter == null) {
        getter = function(x) {
          return x;
        };
      ...
      parent = isClass ? (_ref = this.object.__super__) != null ? _ref.constructor : void 0 : (cons = this.object.constructor).prototype === this.object ? cons.__super__ : cons.prototype;
      if (parent != null) {
        val = getter(parent);
        if (val != null) {
          results.push(val);
  ...
  Batman.Hash = (function() {
    ...
    _ref = ['hasKey', 'equality', 'each', 'keys', 'merge'];
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      k = _ref[_i];
  ...
  Batman.Set = (function() {
    ...
    _ref2 = ['has', 'each', 'isEmpty', 'toArray', 'clear', 'merge'];
  ...
  Batman.Model = (function() {
    ...
    Model.persist = function() {
      var mechanism, mechanisms, storage, _base, _i, _len;
    ...
    Model.load = function(callback) {
      var afterLoad, allMechanisms, fireImmediately, m, mechanisms, _i, _j, _len, _len2;
      ...
      for (_i = 0, _len = allMechanisms.length; _i < _len; _i++) {
        mechanisms = allMechanisms[_i];
    ...
    Model.encode = function() {
      var key, keys, _base, _base2, _i, _len, _results;
      ...
      for (_i = 0, _len = keys.length; _i < _len; _i++) {
        key = keys[_i];
        this.prototype._batman.encoders.set(key, function(value) {
          return '' + value;
        });
        _results.push(this.prototype._batman.decoders.set(key, function(value) {
          return value;
        }));
      }
      return _results;
    ...
    function Model(idOrAttributes) {
      ...
      id = $typeOf(idOrAttributes) === 'String' ? idOrAttributes : idOrAttributes.id;
      if (id != null) {
        this._id("" + id);
    ...
    Model.prototype.toJSON = function() {
      ...
      if (!(!encoders || encoders.isEmpty())) {
        encoders.each(__bind(function(key, encoder) {
          return obj[key] = encoder(this[key]);
        }, this));
      }
      return obj;
    };
    ...
    Model.prototype.validate = function() {
      var async, key, promise, v, validator, _j, _k, _len2, _len3, _ref2, _ref3;
      this.beforeValidation();
      async = false;
      _ref2 = this._batman.get('validators') || [];
      for (_j = 0, _len2 = _ref2.length; _j < _len2; _j++) {
        validator = _ref2[_j];
  ...
  Batman.RestStorage = (function() {
    ...
    RestStorage.prototype.optionsForRecord = function(record) {
      var options, thing, _i, _len, _ref;
      options = {
        type: 'json'
      };
      _ref = [record, this.model];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        thing = _ref[_i];
        if (thing.url) {
          options.url = (typeof thing.url === "function" ? thing.url() : void 0) || thing.url;
        } else {
          options.url = "/" + this.modelKey + "/" + (record._id());
        }
        if (options.url) {
          break;
        }
    ...
    RestStorage.prototype.readAllFromStorage = function(model, callback) {
      var options;
      options = $mixin(this.optionsForRecord(model), {
        success: function(data) {
  ...
  Batman.Renderer = (function() {
    ...
    Renderer.prototype.parseNode = function(node) {
      var attr, index, name, nextNode, result, skipChildren, _base, _base2, _i, _len, _name, _ref, _ref2;
  ...
  Binding = (function() {
    ...
    Binding.prototype.accessor('binding', {
      get: function() {
        ...
        }, this), this.key ? this.keyContext.get(this.key) : this.value);
      }
    });
    function Binding() {
      var shouldSet, unfilteredValue, _ref;
      Binding.__super__.constructor.apply(this, arguments);
      this.parseFilter();
      if (this.key) {
        _ref = this.renderContext.findKey(this.key), unfilteredValue = _ref[0], this.keyContext = _ref[1];
      }
      shouldSet = true;
      if (Batman.DOM.nodeIsEditable(this.node)) {
        Batman.DOM.events.change(this.node, function() {
          shouldSet = false;
          if (this.nodeChange) {
            this.nodeChange(this.node, this.keyContext || this.value);
          } else {
            if (this.key) {
              this.keyContext.set(this.key, this.node.value);
            }
          }
          return shouldSet = true;
        });
      }
      this.observe('binding', true, function(value) {
        if (shouldSet) {
          if (this.dataChange) {
            return this.dataChange(value, this.node);
          } else {
    ...
    Binding.prototype.parseFilter = function() {
      ...
      filters = this.keyPath.replace(/'/g, '"').split(/(?!")\s+\|\s+(?!")/);
      try {
  ...
  RenderContext = (function() {
    ...
    RenderContext.prototype.set = function() {
      var args, _ref;
      args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
      return (_ref = this.storage).set.apply(_ref, args);
    };
    RenderContext.prototype.observe = function(key, observer) {
      return this.findKey(key)[1].observe(key, true, observer);
    };
  ...
  Batman.DOM = {
    readers: {
      ...
      context: function(node, key, context) {
        var subContext, value, _ref;
        _ref = context.findKey(key), value = _ref[0], subContext = _ref[1];
      ...
      route: function(node, key, context) {
        var controller, controllerName, id, index, route, routeName, _ref;
        if (key.substr(0, 1) === '/') {
    ...
    attrReaders: {
      bind: function(node, attr, key, context) {
        return context.bind(node, key, function(value) {
          ...
          return subContext.set(key, value);
        });
      },
      ...
      event: function(node, eventName, key, context) {
        var callback, subContext, _ref;
        if (key.substr(0, 1) === '@') {
          callback = new Function(key.substr(1));
        } else {
          _ref = context.findKey(key), callback = _ref[0], subContext = _ref[1];
        }
      ...
      foreach: function(node, iteratorName, key, context, parentRenderer) {
        var add, collection, nodeMap, parent, prototype, remove, sibling;
        ...
        if (collection != null ? collection.observe : void 0) {
          collection.observe('add', add = function(item) {
            ...
            return renderer = new Batman.Renderer(newNode, function() {
  ...
  filters = Batman.Filters = {
    truncate: function(value, length, end) {
      if (end == null) {
        end = "...";
      }
