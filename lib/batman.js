(function() {
  ...
  Batman.RenderCache = (function() {
    ...
    RenderCache.prototype.maximumSize = 10;
    ...
    RenderCache.prototype.viewForOptions = function(options) {
      var view;
      if (!(view = this._retrieve(options))) {
        view = this._newViewFromOptions(options);
        this._store(view, options);
      }
      return view;
    };

    RenderCache.prototype.cacheKey = function(options) {
      return "" + options.source + $functionName(options.viewClass);
    };

    RenderCache.prototype.length = function() {
    ...
    RenderCache.prototype._store = function(view, options) {
      var indexKey, indexPosition, key, viewPosition, _ref;
      key = this.cacheKey(options);
      if (this.length() === this.maximumSize) {
        viewPosition = this.activityIndex.unshift();
        _ref = this.positionIndex;
        for (indexKey in _ref) {
          indexPosition = _ref[indexKey];
          if (indexPosition = viewPosition) delete this.positionIndex[key];
        }
      } else {
        viewPosition = this.length();
      }
      this.cache[viewPosition] = {
        view: view,
        options: options
      };
      this.positionIndex[key] = viewPosition;
      view;
      if (this.length() === 0) {}
    };

    RenderCache.prototype._retrieve = function(options) {
      var activityPosition, key, position, storage;
      key = this.cacheKey(options);
      position = this.positionIndex[key];
      storage = this.cache[position];
      if ((storage != null) && this._optionsEquivalence(options, storage.options)) {
        activityPosition = this.activityIndex.indexOf(position);
        this.activityIndex.splice(activityPosition, 1);
        this.activityIndex.push(position);
        return storage.view;
      }
    };

    RenderCache.prototype._optionsEquivalence = function(incomingOptions, storageOptions) {
      var key;
  ...
  Batman.DOM = {
    ...
    appendChild: $appendChild = function() {
      ...
      if ((_ref = Batman.data(child, 'show')) != null) _ref.apply(child, args);
