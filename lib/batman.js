...
=======
  ...
  */
  Batman.StateMachine = {
    initialize: function() {
      var _base, _base2;
      Batman._initializeObject(this);
      (_base = this._batman).states || (_base.states = {});
      return (_base2 = this._batman).transitions || (_base2.transitions = {});
    },
    state: function(name, callback) {
      var callbacks, states;
      Batman.StateMachine.initialize.call(this);
      states = this._batman.states;
      callbacks = states[name] || (states[name] = []);
      if (callbacks.indexOf(callback) === -1) {
        return callbacks.push(callback);
      }
    },
    transition: function(from, to, callback) {
      var callbacks, fromIndex, transitions;
      Batman.StateMachine.initialize.call(this);
      transitions = this._batman.transitions;
      fromIndex = transitions[from] || (transitions[from] = {});
      callbacks = fromIndex[to] || (fromIndex[to] = []);
      if (callbacks.indexOf(callback) === -1) {
        return callbacks.push(callback);
      }
    },
    setState: function(name) {
      var callback, callbacks, oldName, _base, _base2, _i, _j, _len, _len2, _ref;
      Batman.StateMachine.initialize.call(this);
      if (this._batman.isTransitioning) {
        return ((_base = this._batman).nextState || (_base.nextState = [])).push(name);
      }
      this._batman.isTransitioning = true;
      (_base2 = this._batman).nextState || (_base2.nextState = []);
      oldName = this._batman.currentState;
      this._batman.currentState = name;
      callbacks = (_ref = this._batman.transitions[oldName]) != null ? _ref[name] : void 0;
      if (callbacks) {
        for (_i = 0, _len = callbacks.length; _i < _len; _i++) {
          callback = callbacks[_i];
          callback.call(this, name, oldName, this);
        }
      }
      callbacks = this._batman.states[name];
      if (callbacks) {
      ...
      if (this._batman.nextState.length) {
        return this.setState(this._batman.nextState.pop());
      }
    },
    currentState: function() {
      return this._batman.currentState;
    }
  };
  /*
  # Batman.Object
  ...
  */
>>>>>>> First pass at Batman.StateMachine.
